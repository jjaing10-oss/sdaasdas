...
function parseKpiExcel(wb, filename){try{
  ...
  var C = 1; // 기본값
  for(var _ri=0;_ri<rowsG.length;_ri++){
    var _r=rowsG[_ri];
    if(typeof _r[0]==='string' && _r[0].length>=2 && _r[0].indexOf(' ')<0){
      var _sc=toNum(_r[1]);
      if(_sc!==null && _sc>=40 && _sc<=120){ C=0; break; }
    }
    if(typeof _r[1]==='string' && _r[1].length>=2 && _r[1].indexOf(' ')<0){
      var _sc2=toNum(_r[2]);
      if(_sc2!==null && _sc2>=40 && _sc2<=120){ C=1; break; }
    }
  }
  // ── 헬퍼: 본부행 판별 ──
  ...
  rowsW.forEach(r=>{
    const nm=hqName(r[C]);
    if(!nm||nm.length<2||nm.indexOf(' ')>=0) return;
    const sc=toNum(r[C+1])||0; const rk=toNum(r[C+2])||0;
    if(sc>=40&&sc<=120&&rk>=1&&rk<=9&&sc>0){
      whlDetail[nm]={total:sc,rank:rk,
        hubul:toNum(r[C+3])||0,mnp:toNum(r[C+4])||0,wired:toNum(r[C+5])||0,mit:toNum(r[C+6])||0,
        rvoc:toNum(r[C+7])||0,oneg:toNum(r[C+8])||0,apru:toNum(r[C+9])||0,
        infra:toNum(r[C+10])||0,add_rev:toNum(r[C+12])||0};
  ...
  rowsS.forEach(r=>{
    ...
    smbDetail[nm]={total:sc,rank:rk,
      hubul:toNum(r[C+4])||0,wired:toNum(r[C+5])||0,mit:toNum(r[C+6])||0,
      strategy:toNum(r[C+7])||0,rvoc:toNum(r[C+8])||0,op_profit:toNum(r[C+9])||0};
  });

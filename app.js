...
function parseKpiExcel(wb, filename){try{
  ...
  if(kpi.length<3){showUpStatus('kpi','err','❌ KPI 데이터 부족 (종합 시트 '+kpi.length+'개)');return;}
  ...
  rowsR.forEach((r,i)=>{
    ...
    if(nm.endsWith('본부')){
      // 소매 본부: col4=후불, 5=MNP, 6=유선, 7=MIT, 8=ARPU, 9=전략, 10=RVOC, 11=1G, 12=TCSI, 14=정책매출, 15=매장이익
  ...
  rowsW.forEach((r,i)=>{
    ...
    const sc=+r[2]||0; const rk=+r[3]||0;
    if(nm.endsWith('본부')){
      // 도매 본부: col4=후불, 5=MNP, 6=유선, 7=MIT, 8=RVOC, 9=1G, 10=ARPU, 11=인프라, 13=추가수익
  ...
  rowsS.forEach((r,i)=>{
    ...
    const sc=+r[2]||0; const rk=+r[3]||0;
    // 소상공인: col4=영업합계, 5=후불, 6=유선, 7=MIT, 8=전략, 9=RVOC, 10=영업이익

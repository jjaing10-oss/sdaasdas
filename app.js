...
function switchTab(btn){
  ...
  if(tabMonth && TAB_DATA_KEYS[t]){
    ...
    // subscriber·ai·upload 등 글로벌 월 표시
...
function parseKpiExcel(wb, filename){try{
  ...
  function sheetRows(name){
    const ws = wb.Sheets[name] || (()=>{
      const keyword = name.replace(/1Q\s*/,'').replace(/\s/g,'');
      const found = wb.SheetNames.find(s=>String(s).replace(/\s/g,'').includes(keyword));
      return found ? wb.Sheets[found] : null;
    })();
    return ws ? XLSX.utils.sheet_to_json(ws,{header:1,defval:0}) : [];
  }
  // ── 헬퍼: 본부명 정규화 ──
  function hqName(v){ return String(v||'').trim(); }
  // ── 헬퍼: 유효 본부 행 여부 ──
  function isHqRow(r){ const n=hqName(r[1]); return n.endsWith('본부'); }
  // ── 헬퍼: 유효 지사 행 여부 ──
  function isJisaRow(r,suffix){ const n=hqName(r[1]); return suffix?n.endsWith(suffix):(n.includes('소매')||n.includes('도매')); }

  // ── 공통: 행에서 본부 데이터 동적 탐색 (고정 인덱스 X) ──
  function findHqRows(rows){ return rows.filter(r=>isHqRow(r)); }
  function findJisaRows(rows,suffix){ return rows.filter(r=>{ const n=hqName(r[1]); return suffix?n.endsWith(suffix):(n.includes('소매')||n.includes('도매')); }); }
  ...
  if(kpi.length<3){showUpStatus('kpi','err','❌ KPI 데이터 부족 (종합 시트 '+kpi.length+'개 / 총'+rowsG.length+'행)');return;}
...
// 탭별로 저장/복원할 D 키 매핑
const TAB_DATA_KEYS = {
  profit:  ['profit','sga_detail','hq','hq_context'],

...
function switchTab(btn){
  ...
  if(tabMonth && TAB_DATA_KEYS[t]){
    ...
    // 업로드 탭 진입 시 기준월 select 초기화
    setTimeout(initUploadMonthSelect, 50);
    updateHdrBadge(D.baseMonth||'2026.01');
  } else if(!TAB_DATA_KEYS[t]){
    // 대시보드·브리핑: D.baseMonth(현재 메모리) 우선, 없으면 저장된 최신 월
    // → 업로드 직후 저장 전이어도 D.baseMonth가 맞는 월을 가리킴
    const storedMonths = getStoredMonths();
    const latestStored = storedMonths.length ? storedMonths[storedMonths.length-1] : null;
    const displayMonth = D.baseMonth || latestStored || '2026.01';
    // 저장된 월이 D보다 최신이면(월 선택기로 이전 월 선택한 경우) 그 스냅샷 복원
    if(latestStored && latestStored > displayMonth){
      const snap = loadMonthFromStorage(latestStored);
      if(snap){ restoreSnapshot(snap); normalizeProfitData(); }
    }
    updateHdrBadge(displayMonth);
  }
  try{
  ...
  <div style="background:#f8fafc;border:1px solid #dbeafe;border-radius:14px;padding:16px;margin-bottom:10px">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:6px">
      <div>
        ...
        <div style="font-size:11px;color:#64748b;margin-top:2px">${(D.baseMonth||'').replace(/^(\d+)\.(0?(\d+))$/,'$1년 $3월')||D.period} · 손익 · KPI · 무선/유선 가입자 · 과제 통합</div>
      </div>
...
// ── 파일명에서 연/월 감지 ──
// ── 업로드 기준월 선택 UI ──
function initUploadMonthSelect(){
  ...
  for(let m=1; m<=12; m++){
    ...
    opt.value = val;
    opt.textContent = '2026년 '+m+'월';
    sel.appendChild(opt);
  }
  // 현재 D.baseMonth에 맞춰 선택
  const cur = D.baseMonth || '2026.01';
  sel.value = cur;
  const badge = document.getElementById('upBasemonthBadge');
  if(badge) badge.textContent = '현재: ' + cur;
}

function onUploadMonthChange(val){
  if(!val) return;
  setGlobalBaseMonth(val, '수동설정');
  const badge = document.getElementById('upBasemonthBadge');
  if(badge) badge.textContent = '현재: ' + val;
}

function getUploadMonth(){
  // 업로드 탭 선택 월 우선, 없으면 D.baseMonth
  const sel = document.getElementById('uploadMonthSelect');
  return (sel && sel.value) ? sel.value : (D.baseMonth || '2026.01');
}
...
function exportDataJson(){
  try{
    const allMonths = getStoredMonths();
    const payload = { _version:1, _exportedAt:new Date().toISOString(), months:{} };
    allMonths.forEach(function(m){
      try{
        const raw = localStorage.getItem(MONTH_STORE_PREFIX+m);
        if(raw) payload.months[m] = JSON.parse(raw);
      }catch(e){}
    });
    // 현재 메모리 D도 포함 (저장 안 된 월 대비)
    const cur = D.baseMonth||'2026.01';
    if(!payload.months[cur]) payload.months[cur] = captureSnapshot();
    try{
      const sub = localStorage.getItem('ktms_subscriber_data');
      if(sub) payload._subscriberData = JSON.parse(sub);
    }catch(e){}
    const months = Object.keys(payload.months).sort();
    const blob = new Blob([JSON.stringify(payload,null,2)],{type:'application/json;charset=utf-8'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'data_latest.json';
    a.click();
    setTimeout(()=>URL.revokeObjectURL(a.href),1000);
    // 버튼 피드백
    const btn = document.querySelector('.mp-action-export');
    if(btn){ const o=btn.textContent; btn.textContent='✅ 저장됨'; setTimeout(()=>btn.textContent=o,1800); }
    const hBtn = document.getElementById('hdrShareBtn');
    if(hBtn){ const o=hBtn.innerHTML; hBtn.innerHTML='✅ 저장됨'; hBtn.style.color='#34d399';
      setTimeout(()=>{ hBtn.innerHTML=o; hBtn.style.color=''; },1800); }
...
function importDataJson(input){
  ...
  reader.onload = function(e){
    try{
      ...
      if(!payload._version||!payload.months){ alert('올바른 KTMS 데이터 파일이 아닙니다.'); return; }
      const months = Object.keys(payload.months).sort();
      const meta = {};
      months.forEach(function(m){
        const snap = payload.months[m];
        if(!snap) return;
        try{
          localStorage.setItem(MONTH_STORE_PREFIX+m, JSON.stringify(snap));
          meta[m]={savedAt:snap._savedAt||Date.now(),taskCount:(snap.tasks||[]).length};
        }catch(e2){}
      });
      _setMonthMeta(meta);
      if(payload._subscriberData){
        try{ localStorage.setItem('ktms_subscriber_data',JSON.stringify(payload._subscriberData)); }catch(e2){}
      }
      const latestM = months[months.length-1];
      if(latestM){
        const snap = payload.months[latestM];
        if(snap){ restoreSnapshot(snap); normalizeProfitData(); }
      }
      try{initTaskUI();}catch(e2){}
      try{initProfitUI();renderTrendChart();}catch(e2){}
      try{initKpiUI();}catch(e2){}
      try{initSubscriberUI();}catch(e2){}
      try{initCommission();}catch(e2){}
      try{initDashboard();}catch(e2){}
      try{initBriefing();}catch(e2){}
      updateHdrBadge(latestM||D.baseMonth||'2026.01');
      closeMonthPicker();
      alert('✅ 데이터 가져오기 완료!\n'+months.length+'개월 데이터 복원 ('+months.join(', ')+')');
    }catch(err){ alert('가져오기 오류: '+err.message); }
    input.value='';
  };
...
async function exportShareableHTML(){
  ...
  try{
    // app.js / style.css / index.html fetch (data.js는 선택적)
    const [appJs, styleCss, dataJs, indexHtml] = await Promise.all([
      ...
      fetch('data.js').then(r=>r.ok?r.text():null).catch(()=>null),  // 없어도 OK
      fetch('index.html').then(r=>r.ok?r.text():null).catch(()=>null),
    ]);
    if(!appJs||!styleCss||!indexHtml){
      alert('소스 파일 로드 실패: ' + [!appJs&&'app.js',!styleCss&&'style.css',!indexHtml&&'index.html'].filter(Boolean).join(', '));
      return;
    }
    ...
    // </script> 가 JSON 안에 있으면 스크립트 태그가 조기 종료됨 → 이스케이프 필수
    const _safeJs = s => s.replace(/<\/script>/gi, '<\\/script>');
    const safeJson = obj => _safeJs(JSON.stringify(obj));
    const safeRaw  = str => _safeJs(JSON.stringify(str));
    const snapJson = safeJson(snap);

    // 스냅샷 주입 스크립트 (최신 월 = 열릴 때 보여줄 데이터)
    const snapScript = '<scr'+'ipt>window.__KTMS_SNAPSHOT__='+snapJson+';<'+'/scr'+'ipt>';

    // 저장된 모든 월 localStorage 복원 스크립트 (경영진이 파일을 열면 1월~현재까지 모두 복원)
    const allMonths = getStoredMonths();
    let allMonthsLsEntries = '';
    allMonths.forEach(function(m){
      try{
        const raw = localStorage.getItem(MONTH_STORE_PREFIX+m);
        if(raw) allMonthsLsEntries += 'localStorage.setItem('+JSON.stringify(MONTH_STORE_PREFIX+m)+','+safeRaw(raw)+');';
      }catch(e){}
    });

    // localStorage 복원 스크립트
    const lsRestore = '<scr'+'ipt>try{' +
      'localStorage.setItem("'+MONTH_META_KEY+'",'+safeRaw(JSON.stringify(_getMonthMeta()))+');' +
      'localStorage.setItem("'+TAB_ACTIVE_KEY+'",'+safeRaw(JSON.stringify(snap._tabActive||{}))+');' +
      allMonthsLsEntries +
      '}catch(e){}<'+'/scr'+'ipt>';

    // 모든 외부 참조 → 완전 인라인 (data.js 없어도 동작, </script> 이스케이프 필수)
    const safeInline = src => src ? src.replace(/<(\/?script)/gi, '<\\$1') : '';
    const dataJsBlock = dataJs
      ? ('<scr'+'ipt>'+safeInline(dataJs)+'<'+'/scr'+'ipt>'+snapScript)
      : (snapScript);  // data.js 없으면 스냅샷만 주입
    let html = indexHtml
      .replace(/<link[^>]*href="style\.css"[^>]*>/i, '<style>'+styleCss+'</style>')
      .replace(/<script[^>]*src="data\.js"[^>]*><\/script>/i, dataJsBlock)
      .replace(/<script[^>]*src="app\.js"[^>]*><\/script>/i, '<scr'+'ipt>'+safeInline(appJs)+'<'+'/scr'+'ipt>')
      .replace(/<\/body>/i, lsRestore+'</body>');

    const blob = new Blob([html], {type:'text/html;charset=utf-8'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'KT_MS_경영관리_'+(allMonths.length>1?allMonths[0]+'~'+allMonths[allMonths.length-1]:D.baseMonth||'2026.01')+'_공유.html';
    a.click();
    setTimeout(()=>URL.revokeObjectURL(a.href), 1000);
  }catch(err){
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  // ── GitHub Pages: data_latest.json 자동 fetch ──
  fetch('data_latest.json?_='+Date.now())
    .then(function(r){ return r.ok?r.json():null; })
    .then(function(payload){
      if(!payload||!payload.months) return;
      const months = Object.keys(payload.months).sort();
      const meta = {};
      months.forEach(function(m){
        const snap = payload.months[m];
      ...
      try{initDashboard();}catch(e){}
      try{initBriefing();}catch(e){}
      console.log('[KTMS] data_latest.json 자동 로드:', latestM);
    })
    .catch(function(){});  // 파일 없으면 조용히 무시

  // ── URL 파라미터 월 지정 ──
  setTimeout(function(){
    try{
      const params = new URLSearchParams(window.location.search);
      const urlMonth = params.get('month')||params.get('m');
      if(urlMonth){
        const stored = getStoredMonths();
        if(stored.includes(urlMonth)) switchToMonth(urlMonth);
        else updateHdrBadge(urlMonth);
      }
    }catch(e){}
  }, 800);
});

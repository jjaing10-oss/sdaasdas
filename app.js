...
async function exportShareableHTML(){
  ...
  try{
    ...
    const safeInline = src => src ? src.replace(/<\/script>/gi, '<\\/script>') : '';
...
// ── 공유 버튼 → 공유용 HTML 내보내기로 연결 ──
function copyShareLink(){
  // URL 링크는 localStorage가 기기별이라 데이터가 전달 안 됨
  // → 공유용 HTML 내보내기(데이터 내장)로 안내
  const btn = document.getElementById('hdrShareBtn');
  exportShareableHTML().then ? exportShareableHTML() : exportShareableHTML();
}

document.addEventListener('DOMContentLoaded',function(){
  setTimeout(initDashboard, 50);
  // ── URL 파라미터로 기본 월 지정 지원 ──
  // 공유 링크 예시: index.html?month=2026.02 → 2월 자동 선택
  setTimeout(function(){
    try{
      ...
      if(urlMonth){
        const stored = getStoredMonths();
        if(stored.includes(urlMonth)){
          switchToMonth(urlMonth);
        } else {
          // 저장된 월이 없어도 배지는 업데이트 (데이터 없음 상태로)
          updateHdrBadge(urlMonth);
        }
      }
    }catch(e){}
  }, 300);

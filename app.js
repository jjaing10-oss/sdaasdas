...
async function exportShareableHTML(){
  const btn = document.querySelector('.mp-action-export');
  if(btn){ btn.textContent = '⏳ 생성 중...'; btn.disabled=true; }
  try{
    // 모든 소스 파일 fetch (data.js 포함)
    ...
    // 내보내기 전 모든 탭 스냅샷을 D에 머지 (최신 데이터 반영)
    const active = _getTabActive();
    ['profit','kpi','tasks','commission'].forEach(function(tab){
      const month = active[tab];
      if(month) restoreTabSnapshot(tab, month);
    });
    normalizeProfitData();
    // D.period도 baseMonth 기준으로 갱신
    ...
    const snap = captureSnapshot();
    try{
      const subRaw = localStorage.getItem('ktms_subscriber_data');
    ...
    // 모든 외부 참조 → 완전 인라인 (toolbar 주입 없이 clean)
    let html = indexHtml
    ...
    if(btn){ btn.textContent = '📤 공유용 HTML 내보내기'; btn.disabled=false; }
  }
}

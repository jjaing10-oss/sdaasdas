...
function exportDataJSON(){
  ...
  try{
    ...
    // factbookUploaded: 문자열이 아닌 parsed object로 저장 (import 시 double-stringify 방지)
    snap._factbookUploaded = uploaded ? JSON.parse(uploaded) : null;
  ...
  reader.onload = function(e){
    try{
      ...
      if(snap._subscriberData){
        ...
        // (= 참조는 유지하면서 내용만 교체 → let/const/var 모두 대응)
        try{
          Object.keys(subscriberData).forEach(function(k){ delete subscriberData[k]; });
          Object.assign(subscriberData, snap._subscriberData);
        }catch(ex){
          // fallback: 재할당 시도
          subscriberData = snap._subscriberData; // eslint-disable-line
        }
        // sourceFile 마킹 보장
        if(!subscriberData.sourceFile) subscriberData.sourceFile = '업로드 Factbook';
        // _dataVersion 주입 (새로고침 후 loadSubscriberData가 복원하도록)
        try{ subscriberData._dataVersion = DEFAULT_SUBSCRIBER_DATA._dataVersion||0; }catch(ex){}
        // saveSubscriberData() 호출 — ktms_subscriber_data + ktms_factbook_uploaded 동시 저장
        saveSubscriberData();

...
function switchTab(btn){
  ...
  // 탭별 저장된 스냅샷 복원 + 배지 갱신
  const tabMonth = getTabMonth(t);
  if(tabMonth && TAB_DATA_KEYS[t]){
    const ok = restoreTabSnapshot(t, tabMonth);
    // commission 탭은 profit 스냅샷에서도 복원 시도
    if(t==='commission'&&!ok){
      const profitMonth=getTabMonth('profit');
      if(profitMonth) restoreTabSnapshot('profit',profitMonth);
    }
    normalizeProfitData();
    updateHdrBadge(tabMonth||(getTabMonth('profit')||D.baseMonth||'2026.01'));
    try{ if(t==='profit'){initProfitUI();renderTrendChart();}
         if(t==='kpi') initKpiUI();
         if(t==='tasks') initTaskUI();
         initDashboard(); }catch(e){}
  } else if(t==='commission'){
    // commission 탭인데 active month 없을 때 → profit 스냅샷에서 commission 복원
    const profitMonth=getTabMonth('profit');
    if(profitMonth){ restoreTabSnapshot('profit',profitMonth); updateHdrBadge(profitMonth); }
    else updateHdrBadge(D.baseMonth||'2026.01');
  } else if(!TAB_DATA_KEYS[t]){
    updateHdrBadge(D.baseMonth||'2026.01');
    if(t==='subscriber') try{ initSubscriberUI(); }catch(e){}
  }
  try{
...
function openReport(html, title){
  ...
  // 편집 툴바 + 플로팅 버튼 자동 주입 (문자열 연결로 템플릿 리터럴 중첩 회피)
  const _sc = '<'+'script>';
  const _esc = '<'+'/script>';
  const toolbar = [
    '<div id="_floatBar">',
    '<button class="fb fb-edit" onclick="_toggleEdit()">&#9999; 편집</button>',
    '<button class="fb fb-save" onclick="_saveHTML()">&#128190; 저장</button>',
    '<button class="fb fb-print" onclick="window.print()">&#128424; 인쇄</button>',
    '</div>',
    '<div id="_editBar">',
    '<button class="tb gold" onclick="_toggleEdit()">&#10003; 편집완료</button>',
    '<div class="tb-sep"></div>',
    '<button class="tb" onclick="document.execCommand(&quot;bold&quot;)"><b>B</b></button>',
    '<button class="tb" onclick="document.execCommand(&quot;italic&quot;)"><i>I</i></button>',
    '<button class="tb" onclick="document.execCommand(&quot;underline&quot;)"><u>U</u></button>',
    '<div class="tb-sep"></div>',
    '<div class="cpill"><span>색상</span><input type="color" id="_cp" value="#003087">',
    '<button class="apply-btn" onclick="document.execCommand(&quot;foreColor&quot;,false,document.getElementById(&quot;_cp&quot;).value)">적용</button></div>',
    '<div class="tb-sep"></div>',
    '<button class="tb" onclick="document.execCommand(&quot;fontSize&quot;,false,&quot;4&quot;)">크게</button>',
    '<button class="tb" onclick="document.execCommand(&quot;fontSize&quot;,false,&quot;2&quot;)">작게</button>',
    '<div class="tb-sep"></div>',
    '<button class="tb green" onclick="_saveHTML()">&#128190; 저장</button>',
    '<button class="tb" style="color:#ff9999;border-color:#7a3a3a" onclick="_resetAll()">&#8617; 원복</button>',
    '</div>',
    _sc,
    'let _editOn=false,_sel=null,_snap=null;',
    'document.addEventListener("DOMContentLoaded",function(){_snap=document.getElementById("_mainContent").innerHTML;});',
    'function saveSel(e){var s=window.getSelection();if(s&&s.rangeCount>0)_sel=s.getRangeAt(0).cloneRange();if(e)e.preventDefault();}',
    'function _toggleEdit(){',
    '  _editOn=!_editOn;',
    '  var mc=document.getElementById("_mainContent");',
    '  var eb=document.getElementById("_editBar");',
    '  var fb=document.getElementById("_floatBar");',
    '  mc.contentEditable=_editOn?"true":"false";',
    '  mc.style.outline=_editOn?"2px dashed #6688cc":"";',
    '  eb.classList.toggle("show",_editOn);',
    '  fb.style.display=_editOn?"none":"flex";',
    '  document.body.classList.toggle("edit-on",_editOn);',
    '  if(_editOn)mc.focus();else _sel=null;',
    '}',
    'function _resetAll(){if(!confirm("모든 편집 내용을 원문으로 복원하시겠습니까?"))return;document.getElementById("_mainContent").innerHTML=_snap;}',
    'function _saveHTML(){',
    '  var eb=document.getElementById("_editBar");',
    '  var fb=document.getElementById("_floatBar");',
    '  var mc=document.getElementById("_mainContent");',
    '  var was=_editOn;',
    '  eb.classList.remove("show");fb.style.display="flex";document.body.classList.remove("edit-on");if(was)mc.style.outline="";',
    '  var dateStr=new Date().toISOString().slice(0,10).replace(/-/g,"");',
    '  var blob=new Blob(["<!DOCTYPE html>\n"+document.documentElement.outerHTML],{type:"text/html;charset=utf-8"});',
    '  var a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="KTM&S_보고서_"+dateStr+".html";a.click();',
    '  setTimeout(function(){URL.revokeObjectURL(a.href);},1000);',
    '  if(was){eb.classList.add("show");fb.style.display="none";document.body.classList.add("edit-on");mc.style.outline="2px dashed #6688cc";}',
    '}',
    _esc
  ].join('\n');
...
function rptStyle(){
  return `<style>
  @import url('https://fonts.googleapis.com/css2?family=Noto+Serif+KR:wght@400;600;700;900&family=Noto+Sans+KR:wght@300;400;500;600;700;800&display=swap');
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  :root{--navy:#0A1628;--blue:#003087;--blue2:#0f4c81;--red:#E4002B;--gold:#C8941A;--gold2:#f0c060;--white:#fff;--bg:#f2f5f9;--text:#1a1a2e;--muted:#64748b;--border:#dde4ee}
  body{font-family:'Noto Sans KR',sans-serif;background:var(--bg);color:var(--text);padding:32px 24px 80px;font-size:12px;line-height:1.65}
  body.edit-on{padding-top:62px}
  .page{max-width:1060px;margin:0 auto}

  /* ── 플로팅 툴바 ── */
  #_floatBar{position:fixed;top:14px;right:20px;z-index:9999;display:flex;gap:8px}
  #_editBar{position:fixed;top:0;left:0;right:0;z-index:9999;background:#0f1a2e;border-bottom:2px solid var(--gold);padding:8px 18px;display:none;align-items:center;gap:6px;flex-wrap:wrap;font-family:'Noto Sans KR',sans-serif;box-shadow:0 2px 16px rgba(0,0,0,.4)}
  #_editBar.show{display:flex}
  .tb{background:#1a3a5c;border:1px solid #2a5080;color:#c8d8f0;border-radius:5px;padding:4px 12px;font-size:11.5px;cursor:pointer;font-family:'Noto Sans KR',sans-serif;transition:all .15s;line-height:1.4}
  .tb:hover{background:#2a5080;color:#fff;border-color:var(--gold)}
  .tb.gold{background:#3a2a00;border-color:var(--gold);color:var(--gold2);font-weight:700}
  .tb.green{background:#1b4a20;border-color:#2e7d32;color:#a0f0a0}
  .tb-sep{width:1px;height:20px;background:#2a5080;margin:0 3px}
  .cpill{display:flex;align-items:center;gap:5px;background:#1a3a5c;border:1px solid #2a5080;border-radius:5px;padding:3px 9px}
  .cpill span{font-size:11px;color:#8899bb}
  .cpill input[type=color]{width:24px;height:20px;border:none;background:none;cursor:pointer;padding:0}
  .fb{padding:8px 18px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR',sans-serif;border:1.5px solid transparent;box-shadow:0 4px 16px rgba(0,0,0,.25);transition:all .15s;white-space:nowrap}
  .fb-edit{background:var(--blue2);color:#fff;border-color:#3a80c0}
  .fb-save{background:#1b5e20;color:#fff;border-color:#2e7d32}
  .fb-print{background:var(--navy);color:#c8d8f0;border-color:#2a5080}
  .fb:hover{opacity:.88;transform:translateY(-1px)}

  /* ── 헤더 ── */
  .cover{background:linear-gradient(135deg,var(--navy) 0%,#0f2e54 50%,#082040 100%);border-radius:16px;padding:36px 48px;margin-bottom:24px;position:relative;overflow:hidden;color:#fff}
  .cover::before{content:'';position:absolute;top:0;left:0;right:0;height:5px;background:linear-gradient(90deg,var(--red) 0%,var(--gold) 50%,var(--red) 100%)}
  .cover-tag{display:inline-flex;align-items:center;justify-content:center;padding:4px 14px;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.25);border-radius:999px;font-size:10px;font-weight:800;letter-spacing:1px;text-transform:uppercase;margin-bottom:14px}
  .cover-title{font-family:'Noto Serif KR',serif;font-size:26px;font-weight:900;line-height:1.2;margin-bottom:8px}
  .cover-title span{color:var(--gold2)}
  .cover-sub{font-size:12px;color:rgba(255,255,255,.65);line-height:1.7;margin-bottom:20px}
  .cover-meta{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;border-top:1px solid rgba(255,255,255,.18);padding-top:18px}
  .cover-kpi{text-align:center;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:14px 10px}
  .cover-kpi-v{font-size:24px;font-weight:900;letter-spacing:-1px}
  .cover-kpi-l{font-size:10px;color:rgba(255,255,255,.65);font-weight:600;margin-top:3px}

  /* ── 섹션 카드 ── */
  .card{background:#fff;border-radius:12px;box-shadow:0 2px 12px rgba(15,23,42,.08);padding:22px 26px;margin-bottom:18px;border:1px solid var(--border)}
  .card-title{font-size:12.5px;font-weight:700;color:var(--blue2);letter-spacing:.05em;margin-bottom:14px;display:flex;align-items:center;gap:8px;padding-bottom:10px;border-bottom:1.5px solid var(--border)}
  .card-ico{width:26px;height:26px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0}
  .two-col{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:18px}
  .three-col{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:18px}

  /* ── 타이포 ── */
  h1{font-family:'Noto Serif KR',serif;font-size:17px;font-weight:900;color:var(--navy);border-bottom:2px solid var(--gold);padding-bottom:8px;margin:28px 0 14px}
  h1:first-of-type{margin-top:0}
  h2{font-size:13.5px;font-weight:700;color:var(--blue2);margin:18px 0 10px;padding-left:10px;border-left:3px solid var(--blue2)}
  h3{font-size:12px;font-weight:700;color:#334155;margin:12px 0 6px;padding-left:8px;border-left:2px solid var(--gold)}

  /* ── 인사이트 박스 ── */
  .exec-box{background:linear-gradient(135deg,#eef4ff,#dce8f8);border:1px solid #b8d0f0;border-radius:10px;padding:16px 18px;margin:12px 0;position:relative}
  .exec-box::before{content:'EXECUTIVE SUMMARY';position:absolute;top:-9px;left:14px;background:var(--blue2);color:#fff;font-size:8px;font-weight:700;letter-spacing:1.5px;padding:2px 8px;border-radius:4px}
  ...
  .warn{background:#fef2f2;border-left:4px solid var(--red);padding:11px 14px;margin:10px 0;border-radius:0 8px 8px 0;font-size:11.5px;color:#7f1d1d}
  ...
  .caution{background:#fffbeb;border-left:4px solid var(--gold);padding:11px 14px;margin:10px 0;border-radius:0 8px 8px 0;font-size:11.5px;color:#78350f}
  ...
  td{padding:8px 10px;text-align:right;border-bottom:1px solid #f1f5f9;font-size:11px}
  td:first-child{text-align:left;font-weight:600;font-size:11.5px}
  tr:nth-child(even){background:#f8fafc}
  tr:hover{background:#eef4ff}
  tr.total{background:#dbeafe;font-weight:700}
  .pos{color:#059669;font-weight:600}.neg{color:var(--red);font-weight:600}
  .hi{color:var(--blue2);font-weight:700}.lo{color:var(--red)}
  ...
  .stat-label{font-size:9.5px;font-weight:700;color:var(--blue2);letter-spacing:1px;text-transform:uppercase;margin:14px 0 6px;padding:4px 0;border-bottom:1px solid var(--border)}
  .stat-card{border-radius:10px;padding:14px 16px;text-align:center;position:relative;overflow:hidden}
  .stat-card::before{content:'';position:absolute;top:0;left:0;right:0;height:3px}
  .stat-card-blue{background:#f0f9ff;border:1px solid #bae6fd}.stat-card-blue::before{background:var(--blue2)}
  ...
  .stat-card-gold{background:#fffbeb;border:1px solid #fde68a}.stat-card-gold::before{background:var(--gold)}
  .stat-v{font-size:24px;font-weight:900;letter-spacing:-1px;margin:4px 0}
  .stat-v-blue{color:var(--blue2)}.stat-v-green{color:#059669}.stat-v-gray{color:#475569}
  .stat-v-best{color:#16a34a}.stat-v-worst{color:#ea580c}.stat-v-gold{color:var(--gold)}
  ...
  /* ── 기타 ── */
  .ch-cost-block{margin-bottom:18px;border:1px solid var(--border);border-radius:10px;overflow:hidden}
  ...
  .badge-r{background:#dcfce7;color:#16a34a}.badge-y{background:#fef9c3;color:#ca8a04}.badge-o{background:#ffedd5;color:#c2410c}.badge-red{background:#fee2e2;color:var(--red)}
  ...
  .notice-box{background:linear-gradient(90deg,#fff8e1,#fffdf0);border-left:4px solid var(--gold);border-radius:0 8px 8px 0;padding:12px 16px;font-size:11px;color:#78350f;margin-top:18px}
  .priority-table th{font-size:9.5px}.priority-table td{font-size:10.5px}

  /* ── 프린트 ── */
  @media print{
    body{background:#fff;padding:0}
    #_floatBar,#_editBar{display:none!important}
    .cover{border-radius:0;break-after:avoid-page}
    .card{box-shadow:none;break-inside:avoid}
  }
  @media(max-width:860px){
    body{padding:16px 12px 60px}
    .cover{padding:24px 20px;border-radius:12px}
...
// ── 가입자 선택 월 인덱스 공통 헬퍼 (드롭다운 > 헤더배지 > 마지막월) ──
function getSubMonthIdx(months){
  const len=(months||[]).length;
  if(!len) return 0;
  try{
    if(typeof window.__subMonthIdx==='number' && window.__subMonthIdx>=0 && window.__subMonthIdx<len) return window.__subMonthIdx;
    if(typeof D!=='undefined' && D.baseMonth){
      const bm=String(D.baseMonth).match(/^20(\d{2})\.0?(\d+)/);
      if(bm){ const n=`${bm[1]}.${parseInt(bm[2])}월`; const fi=(months).findIndex(m=>m.replace(/'/g,'').trim()===n); if(fi>=0) return fi; }
    }
  }catch(e){}
  return len-1;
}
function buildBriefingEngine(){
  ...
  const _sdMonths = SD2.months||[];
  let lastI2 = getSubMonthIdx(_sdMonths);
...
function getSavedSubscriberData(){
  // ① 업로드 전용 키 우선 확인 (버전과 무관하게 항상 우선 적용)
  try{
    const uploaded = localStorage.getItem('ktms_factbook_uploaded');
    if(uploaded){
      const parsed = JSON.parse(uploaded);
      if(parsed && parsed.months && parsed.wireless) return parsed;
    }
  }catch(e){}
  // ② fallback: 기존 키
...
function saveSubscriberData(){
  try{
    const str = JSON.stringify(subscriberData);
    localStorage.setItem('ktms_subscriber_data', str);
    // 업로드 데이터라면 별도 키에도 저장
    if(subscriberData && subscriberData.sourceFile==='업로드 Factbook'){
      localStorage.setItem('ktms_factbook_uploaded', str);
    }
  }catch(e){
    console.warn('[saveSubscriberData] 저장 실패:', e.message);
  }
}
function loadSubscriberData(){
  const saved=getSavedSubscriberData();
  const defVer = DEFAULT_SUBSCRIBER_DATA._dataVersion||0;
  // 업로드 Factbook 데이터 또는 버전 일치 시 복원
  const isUploaded = saved && saved.sourceFile==='업로드 Factbook' && saved.months && saved.wireless;
  const isVersionMatch = saved && saved._dataVersion===defVer && saved.months && saved.wireless;
  if(isUploaded || isVersionMatch){
...
function clearUploadedFactbook(){
  try{ localStorage.removeItem('ktms_factbook_uploaded'); }catch(e){}
}
function resetSubscriberData(){
  clearUploadedFactbook();
  subscriberData = cloneObj(DEFAULT_SUBSCRIBER_DATA);
  saveSubscriberData();
  initSubscriberUI();
...
// 헤더 행에서 'YY.M월 패턴 셀만 골라 {months, cols} 반환 (연간계·목표 열 자동 제외)
function detectMonthCols(ws, headerRow, scanEnd){
  scanEnd = scanEnd || 80;
  const pat = /\d{2}\.\d{1,2}월/;
  const months=[], cols=[];
  for(let c=0;c<=scanEnd;c++){
    const val = String(pickCell(ws,headerRow,c)||'').replace(/'/g,'').trim();
    if(pat.test(val)){ months.push(val); cols.push(c); }
  }
  return {months, cols};
}
function pickSeriesByCols(ws, row, cols){
  return cols.map(c=>Number(pickCell(ws,row,c))||0);
}
...
function extractSubscriberDataFromWorkbook(wb){
  ...
  // ── 2.무선가입자: 헤더 행6(xlsx_r=5), 월별 컬럼 동적 탐지 ──
  const {months, cols: mColsW} = detectMonthCols(wsW, 5);
  // 일반후불(전체) 섹션 (Excel행 7~16)
  const wirelessTotal    = pickSeriesByCols(wsW,  6, mColsW); // 행7 유지
  const wirelessCapa     = pickSeriesByCols(wsW,  7, mColsW); // 행8 CAPA
  const wirelessNew      = pickSeriesByCols(wsW,  8, mColsW); // 행9 신규
  const wirelessChg      = pickSeriesByCols(wsW,  9, mColsW); // 행10 기변
  const wirelessCancel   = pickSeriesByCols(wsW, 10, mColsW); // 행11 해지
  const wirelessChgOut   = pickSeriesByCols(wsW, 12, mColsW); // 행13 기변OUT
  const wirelessMaturity = pickSeriesByCols(wsW, 13, mColsW); // 행14 만기환수
  const wirelessNetRef   = pickSeriesByCols(wsW, 14, mColsW); // 행15 영업순증
  // 관리수수료(8.5%) 섹션 (Excel행 17~26)
  const wirelessMgmtEligible = pickSeriesByCols(wsW, 16, mColsW); // 행17 유지
  const mgmtCapa     = pickSeriesByCols(wsW, 17, mColsW); // 행18 CAPA
  const mgmtChurn    = pickSeriesByCols(wsW, 20, mColsW); // 행21 해지
  const mgmtChgOut   = pickSeriesByCols(wsW, 22, mColsW); // 행23 기변OUT
  const mgmtMaturity = pickSeriesByCols(wsW, 23, mColsW); // 행24 만기환수
  const mgmtTransfer = pickSeriesByCols(wsW, 24, mColsW); // 행25 가입자이관
  const mgmtNetAdd   = pickSeriesByCols(wsW, 25, mColsW); // 행26 순증
  // H/C(2%) 섹션 (Excel행 27~36)
  const wirelessHcEligible = pickSeriesByCols(wsW, 26, mColsW); // 행27 유지
  const hcCapa     = pickSeriesByCols(wsW, 27, mColsW); // 행28 CAPA
  const hcChurn    = pickSeriesByCols(wsW, 30, mColsW); // 행31 해지
  const hcChgOut   = pickSeriesByCols(wsW, 32, mColsW); // 행33 기변OUT
  const hcMaturity = pickSeriesByCols(wsW, 33, mColsW); // 행34 만기환수
  const hcTransfer = pickSeriesByCols(wsW, 34, mColsW); // 행35 가입자이관
  const hcNetAdd   = pickSeriesByCols(wsW, 35, mColsW); // 행36 순증

  // ── 3.유선가입자: 동적 컬럼 탐지 ──
  const {cols: mColsV} = detectMonthCols(wsV, 5);
  const wiredTotal    = pickSeriesByCols(wsV,  6, mColsV); // 행7 유지
  const internet      = pickSeriesByCols(wsV, 13, mColsV); // 행14 인터넷유지
  const tv            = pickSeriesByCols(wsV, 20, mColsV); // 행21 TV유지
  const wiredNew      = pickSeriesByCols(wsV,  7, mColsV); // 행8 신규
  const wiredCancel   = pickSeriesByCols(wsV,  8, mColsV); // 행9 해지
  const wiredMaturity = pickSeriesByCols(wsV, 11, mColsV); // 행12 만기환수
  const wiredNet      = pickSeriesByCols(wsV, 10, mColsV); // 행11 영업순증
  const wiredNetPure  = pickSeriesByCols(wsV, 12, mColsV); // 행13 순증
  // 인터넷 세부
  const iNew    = pickSeriesByCols(wsV, 14, mColsV); // 행15 인터넷_신규
  const iCancel = pickSeriesByCols(wsV, 15, mColsV); // 행16 인터넷_해지
  const iNetOp  = pickSeriesByCols(wsV, 17, mColsV); // 행18 인터넷_영업순증
  const iMat    = pickSeriesByCols(wsV, 18, mColsV); // 행19 인터넷_만기환수
  const iNet    = pickSeriesByCols(wsV, 19, mColsV); // 행20 인터넷_순증
  // TV 세부
  const tvNew    = pickSeriesByCols(wsV, 21, mColsV); // 행22 TV_신규
  const tvCancel = pickSeriesByCols(wsV, 22, mColsV); // 행23 TV_해지
  const tvNetOp  = pickSeriesByCols(wsV, 24, mColsV); // 행25 TV_영업순증
  const tvMat    = pickSeriesByCols(wsV, 25, mColsV); // 행26 TV_만기환수
  const tvNet    = pickSeriesByCols(wsV, 26, mColsV); // 행27 TV_순증
  // 채널별 순신규 (행29~34 = xlsx_r=28~33, C열=col2 기준)
  const vChNetAdd = {
    '소매':    [pickSeriesByCols(wsV, 29, mColsV)], // 행30
    '소상공인': [pickSeriesByCols(wsV, 30, mColsV)], // 행31
    '도매':    [pickSeriesByCols(wsV, 31, mColsV)], // 행32
    '디지털':  [pickSeriesByCols(wsV, 32, mColsV)], // 행33
  };
  const vChNetAddItv = {
    '소매':    pickSeriesByCols(wsV, 29, mColsV),
    '소상공인': pickSeriesByCols(wsV, 30, mColsV),
    '도매':    pickSeriesByCols(wsV, 31, mColsV),
    '디지털':  pickSeriesByCols(wsV, 32, mColsV),
  };
  const vChNetAddInternet = {
    '소매':    pickSeriesByCols(wsV, 35, mColsV),
    '소상공인': pickSeriesByCols(wsV, 36, mColsV),
    '도매':    pickSeriesByCols(wsV, 37, mColsV),
    '디지털':  pickSeriesByCols(wsV, 38, mColsV),
  };
  const vChNetAddTv = {
    '소매':    pickSeriesByCols(wsV, 45, mColsV),
    '소상공인': pickSeriesByCols(wsV, 46, mColsV),
    '도매':    pickSeriesByCols(wsV, 47, mColsV),
    '디지털':  pickSeriesByCols(wsV, 48, mColsV),
  };

  // ── 채널별 CAPA (Excel행 51~55, xlsx_r=50~54) ──
  const channelSeries={
    '소매':    pickSeriesByCols(wsW, 50, mColsW),
    '도매':    pickSeriesByCols(wsW, 51, mColsW),
    '디지털':  pickSeriesByCols(wsW, 52, mColsW),
    '소상공인': pickSeriesByCols(wsW, 53, mColsW),
    'B2B':    pickSeriesByCols(wsW, 54, mColsW),
  ...
  // 목표 컬럼: 헤더에서 '목표' 키워드가 포함된 첫 번째 컬럼 탐지
  const tgtCol=(()=>{
    for(let c=mColsW[mColsW.length-1]+1; c<=mColsW[mColsW.length-1]+5; c++){
      const v=String(pickCell(wsW,5,c)||'');
      if(v.includes('목표')) return c;
    }
    return mColsW[mColsW.length-1]+2; // fallback
  })();
  const channelCapaTarget={
    '소매':    Number(pickCell(wsW, 50, tgtCol))||0,
    '도매':    Number(pickCell(wsW, 51, tgtCol))||0,
    '디지털':  Number(pickCell(wsW, 52, tgtCol))||0,
    '소상공인': Number(pickCell(wsW, 53, tgtCol))||0,
    'B2B':    Number(pickCell(wsW, 54, tgtCol))||0,
    '기타':0
  };
  // ── ARPU (Excel행 90~95, xlsx_r=89~94) ──
  const arpuOverall = pickSeriesByCols(wsW, 89, mColsW);
  const channelArpuSeries = {
    '소매':    pickSeriesByCols(wsW, 90, mColsW),
    '도매':    pickSeriesByCols(wsW, 91, mColsW),
    '디지털':  pickSeriesByCols(wsW, 92, mColsW),
    '소상공인': pickSeriesByCols(wsW, 93, mColsW),
    'B2B':    pickSeriesByCols(wsW, 94, mColsW),
    '기타': new Array(months.length).fill(0)
  };

  // ── 2-(1) 채널별 관리수수료/H.C 분해: 월 컬럼 동적 탐지 후 months 기준으로 슬라이싱 ──
  let mgmtChannels={}, hcChannels={};
  if(ws21){
    // 2-(1) 헤더 행6(xlsx_r=5)에서 'YY.M월 컬럼 목록 탐지
    const {months: m21, cols: c21} = detectMonthCols(ws21, 5, 90);
    // 메인 months 레이블과 교집합: 같은 월 레이블을 가진 컬럼만 사용
    const monthSet = new Set(months.map(m=>m.replace(/'/g,'').trim()));
    const filteredCols = c21.filter((_,i)=>monthSet.has(m21[i].replace(/'/g,'').trim()));
    // 데이터가 없으면 25년 마지막 12개 컬럼 fallback
    const use21 = filteredCols.length > 0 ? filteredCols : c21.slice(-Math.min(12,c21.length));
    const n21 = months.length;
    const pad = (arr)=>{ while(arr.length<n21) arr.push(0); return arr.slice(0,n21); };
    mgmtChannels={
      '소매8.5%':{ total:pad(pickSeriesByCols(ws21,35,use21)), capa:pad(pickSeriesByCols(ws21,36,use21)), churn:pad(pickSeriesByCols(ws21,39,use21)), chgOut:pad(pickSeriesByCols(ws21,41,use21)), maturity:pad(pickSeriesByCols(ws21,42,use21)), netAdd:pad(pickSeriesByCols(ws21,44,use21)) },
      '소상공인8.5%':{ total:pad(pickSeriesByCols(ws21,45,use21)), capa:pad(pickSeriesByCols(ws21,46,use21)), churn:pad(pickSeriesByCols(ws21,49,use21)), netAdd:pad(pickSeriesByCols(ws21,53,use21)) },
      '도매8.5%':{ total:pad(pickSeriesByCols(ws21,64,use21)), capa:new Array(n21).fill(0), netAdd:new Array(n21).fill(0) },
      '온라인제휴8.5%':{ total:pad(pickSeriesByCols(ws21,74,use21)), capa:new Array(n21).fill(0), netAdd:pad(pickSeriesByCols(ws21,83,use21)) },
      'B2B8.5%':{ total:pad(pickSeriesByCols(ws21,94,use21)), capa:pad(pickSeriesByCols(ws21,95,use21)), churn:pad(pickSeriesByCols(ws21,98,use21)), netAdd:pad(pickSeriesByCols(ws21,103,use21)) },
    };
    hcChannels={
      '도매H/C2%':{ total:pad(pickSeriesByCols(ws21,54,use21)), capa:pad(pickSeriesByCols(ws21,55,use21)), churn:pad(pickSeriesByCols(ws21,58,use21)), chgOut:pad(pickSeriesByCols(ws21,60,use21)), maturity:pad(pickSeriesByCols(ws21,61,use21)), netAdd:pad(pickSeriesByCols(ws21,63,use21)) },
      'KT닷컴H/C2%':{ total:pad(pickSeriesByCols(ws21,84,use21)), capa:pad(pickSeriesByCols(ws21,85,use21)), churn:pad(pickSeriesByCols(ws21,88,use21)), chgOut:pad(pickSeriesByCols(ws21,90,use21)), maturity:pad(pickSeriesByCols(ws21,91,use21)), netAdd:pad(pickSeriesByCols(ws21,93,use21)) },
    };
  }
  ...
  const parseOrgBlock=(ws,names,rStart,rEnd,dynCols,tgtCol,y24Col)=>{
    // dynCols: 동적 탐지된 월 컬럼 인덱스 배열
    // tgtCol: 목표 컬럼 인덱스 (없으면 마지막 dynCol 다음 컬럼)
    const nCols = dynCols ? dynCols.length : 12;
    const _tgt = tgtCol != null ? tgtCol : (dynCols ? dynCols[dynCols.length-1]+2 : 18);
    const _y24 = y24Col != null ? y24Col : (dynCols ? dynCols[0]-1 : 5);
    const result={};
    names.forEach(nm=>{
      const r=findLabelRow(ws,nm,rStart,rEnd,1);
      if(r>=0){
        result[nm]={
          series: dynCols ? pickSeriesByCols(ws,r,dynCols) : pickSeries(ws,r,6,6+nCols-1),
          target: Number(pickCell(ws,r,_tgt))||0,
          y24: Number(pickCell(ws,r,_y24))||0
        };
      } else {
  ...
  const wRetailHQ  = parseOrgBlock(wsW, wRetailHQNames,     57, 73, mColsW, tgtCol);
  const wRetailBr  = parseOrgBlock(wsW, wRetailBranchNames, 57, 73, mColsW, tgtCol);
  const wWholesaleHQ = parseOrgBlock(wsW, wWholesaleHQNames,   74, 88, mColsW, tgtCol);
  const wWholesaleBr = parseOrgBlock(wsW, wWholesaleBranchNames, 74, 88, mColsW, tgtCol);

  // 유선: 소매(행52~67=r51~66), 도매(행69~82=r68~81)
  const vRetailHQ   = parseOrgBlock(wsV, wRetailHQNames,       51, 67, mColsV);
  const vRetailBr   = parseOrgBlock(wsV, wRetailBranchNames,   51, 67, mColsV);
  const vWholesaleHQ  = parseOrgBlock(wsV, wWholesaleHQNames,  68, 82, mColsV);
  const vWholesaleBr  = parseOrgBlock(wsV, wWholesaleBranchNames, 68, 82, mColsV);
  ...
  months.forEach((mo,idx)=>{
    const moStr = mo.replace(/'/g,'').trim(); // e.g. '25.1월' → '25.1월'
    const yearNum = moStr.startsWith('26.') ? 2026 : moStr.startsWith('25.') ? 2025 : moStr.startsWith('24.') ? 2024 : 2025;
    const monthNum = parseInt((moStr.match(/\.(\d+)월/)||[])[1]||String(idx+1));
    const year=yearNum, month=monthNum;
  ...
  return {
    ...
    wireless:{
      ...
      org: wirelessOrg,
      // 전년 계: 첫 번째 월 컬럼 바로 앞 컬럼 (e.g. '25년 계)
      prevYearEndTotal: Number(pickCell(wsW, 6, mColsW[0]-1))||0,
      prevYearLabel: String(pickCell(wsW, 5, mColsW[0]-1)||'전년 계').replace(/'/g,'').trim(),
    ...
    y2024:{
      wireless:{
        total:      Number(pickCell(wsW, 6,  mColsW[0]-2))||0,
        capa:       Number(pickCell(wsW, 7,  mColsW[0]-2))||0,
        newSubs:    Number(pickCell(wsW, 8,  mColsW[0]-2))||0,
        chgSubs:    Number(pickCell(wsW, 9,  mColsW[0]-2))||0,
        churn:      Number(pickCell(wsW, 10, mColsW[0]-2))||0,
        chgOut:     Number(pickCell(wsW, 12, mColsW[0]-2))||0,
        maturity:   Number(pickCell(wsW, 13, mColsW[0]-2))||0,
        netAdd:     Number(pickCell(wsW, 14, mColsW[0]-2))||0,
        mgmtEligible: Number(pickCell(wsW, 16, mColsW[0]-2))||0,
        hcEligible:   Number(pickCell(wsW, 26, mColsW[0]-2))||0,
        chSeries:{ 소매: Number(pickCell(wsW,50,mColsW[0]-2))||0, 도매: Number(pickCell(wsW,51,mColsW[0]-2))||0, 디지털: Number(pickCell(wsW,52,mColsW[0]-2))||0, 소상공인: Number(pickCell(wsW,53,mColsW[0]-2))||0, B2B: Number(pickCell(wsW,54,mColsW[0]-2))||0 },
        arpu: Number(pickCell(wsW, 89, mColsW[0]-2))||0,
      },
      wired:{
        total:    Number(pickCell(wsV, 6,  mColsV[0]-1))||0,
        internet: Number(pickCell(wsV, 13, mColsV[0]-1))||0,
        tv:       Number(pickCell(wsV, 20, mColsV[0]-1))||0,
        newSubs:  Number(pickCell(wsV, 7,  mColsV[0]-1))||0,
        cancelSubs: Number(pickCell(wsV, 8, mColsV[0]-1))||0,
        maturity: Number(pickCell(wsV, 11, mColsV[0]-1))||0,
        netAdd:   Number(pickCell(wsV, 10, mColsV[0]-1))||0,
        netPure:  Number(pickCell(wsV, 12, mColsV[0]-1))||0,
        chItv:{ 소매: Number(pickCell(wsV,29,mColsV[0]-1))||0, 소상공인: Number(pickCell(wsV,30,mColsV[0]-1))||0, 도매: Number(pickCell(wsV,31,mColsV[0]-1))||0, 디지털: Number(pickCell(wsV,32,mColsV[0]-1))||0 },
      }
    }
  };
}
...
function buildSubscriberReportHtml(){
  ...
  const months = d.months||[];
  let lastIdx = getSubMonthIdx(months);
...
function buildSubscriberAggregate(d){
  ...
  // ── 선택 월 인덱스 (드롭다운 > 헤더배지 > 마지막월) ──
  let idx = getSubMonthIdx(months);
  // ────────────────────────────────────────────────────────────────

  const currentMonth=(months[idx]||d.baseMonth||'당월').replace(/'/g,'').trim();
  const wirelessRecs=records.filter(r=>r.channel==='무선');
  const m251=wirelessRecs[0]||{};
  const mCur=wirelessRecs[idx]||wirelessRecs[wirelessRecs.length-1]||{};
  const prev=wirelessRecs[Math.max(0,idx-1)]||{};
  // YoY: 현재 월 레이블에서 전년도 같은 달 레코드 탐색
  const curLabel=(months[idx]||'').replace(/'/g,'').trim();
  const yoyLabel=curLabel.replace(/^(\d+)\./,(m,y)=>(parseInt(y)-1)+'.');
  const yoyRec=wirelessRecs.find(r=>(r.monthLabel||'').replace(/'/g,'').trim()===yoyLabel)||null;
  const y24=yoyRec?(yoyRec.endingSubs||0):(Number(((d||{}).wireless||{}).prevYearEndTotal||0)||Number((((d||{}).wireless||{}).series||{}).total?.[0]||0));
  const prevYearEnd=Number(((d||{}).wireless||{}).prevYearEndTotal||0)||(Number((((d||{}).wireless||{}).series||{}).total?.[0]||0));
  const prevYearLabel=String(((d||{}).wireless||{}).prevYearLabel||'전년 계');
  return {schema,records,months,currentMonth,m251,mCur,prev,y24,idx,prevYearEnd,prevYearLabel,wirelessRecs,yoyLabel,curLabel};
}
function renderSubscriberTable(rows, cols){
  return `<div style="overflow-x:auto;-webkit-overflow-scrolling:touch"><table style="min-width:100%;border-collapse:collapse;font-size:11px;table-layout:auto;white-space:nowrap"><thead><tr style="background:#0f172a;color:#fff">${cols.map(c=>`<th style="padding:7px 10px;text-align:${c.align||'right'};white-space:nowrap">${c.label}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${cols.map(c=>`<td style="padding:7px 10px;border-bottom:1px solid #1f2937;text-align:${c.align||'right'};font-weight:${c.align==='left'?'700':'600'};white-space:nowrap">${c.f(r)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
}
function renderAnnualTable(rows, cols){
  return `<div style="overflow-x:auto;-webkit-overflow-scrolling:touch"><table style="min-width:100%;border-collapse:collapse;font-size:14px;table-layout:auto;white-space:nowrap"><thead><tr style="background:#0f172a;color:#94a3b8;font-size:12px">${cols.map(c=>`<th style="padding:10px 14px;text-align:${c.align||'right'};white-space:nowrap;font-weight:600;letter-spacing:.3px">${c.label}</th>`).join('')}</tr></thead><tbody>${rows.map((r,ri)=>`<tr style="background:${ri%2===0?'rgba(255,255,255,.03)':'transparent'}">${cols.map(c=>`<td style="padding:11px 14px;border-bottom:1px solid #1e293b;text-align:${c.align||'right'};font-weight:${c.align==='left'?'700':'600'};white-space:nowrap">${c.f(r)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
}
...
function initSubscriberUI(){
  ...
  // 동적 라벨
  const lbl1=a.prevYearLabel||(a.months[0]||'').replace(/(\d+)\..*/,'$1년 계')||'전년 계';
  const lbl2=(a.months[0]||'25.1월').replace(/'/g,'').trim();
  const lbl3=a.currentMonth;
  const lbl4=`YoY${a.yoyLabel?' vs '+a.yoyLabel:''}`;
  const prevYearEndVal=a.prevYearEnd||a.y24||0;
  ...
  // ── 연 마감별 집계 헬퍼 ──────────────────────────────────────────
  // type: 'stock'=연말 마지막값, 'flow'=합산, 'avg'=평균(0제외)
  function buildAnnualRows(months, seriesMap){
    // 월 레이블 → 연도 추출
    const getYear = m => parseInt((m||'').replace(/'/g,'').trim().match(/^(\d+)\./)?.[1]||'0')+2000;
    // 연도별 인덱스 그룹화
    const yearMap = {};
    months.forEach((m,i)=>{
      const y = getYear(m);
    ...
    return Object.entries(yearMap).map(([year, idxArr])=>{
      const row = {year: year+'년'};
      Object.entries(seriesMap).forEach(([key,{arr,type}])=>{
        const vals = idxArr.map(i=>Number(arr[i]||0));
        if(type==='stock') row[key] = vals[vals.length-1]||0;
        else if(type==='avg'){
          const nonZero = vals.filter(v=>v>0);
          row[key] = nonZero.length ? Math.round(nonZero.reduce((s,v)=>s+v,0)/nonZero.length) : 0;
        } else row[key] = vals.reduce((s,v)=>s+v,0); // flow = sum
      });
      row._months = idxArr.length; // 해당 연도에 포함된 월 수
      return row;
    });
  }

  // ── 무선 연 마감 테이블 ──────────────────────────────────────────
  const wSer = (d.wireless||{}).series||{};
  const annualWireless = buildAnnualRows(a.months,{
    유지:    {arr:wSer.total||[],    type:'stock'},
    CAPA:    {arr:wSer.capa||[],     type:'flow'},
    신규:    {arr:wSer.newSubs||[],  type:'flow'},
    기변:    {arr:wSer.chgSubs||[],  type:'flow'},
    해지:    {arr:wSer.churn||[],    type:'flow'},
    기변OUT: {arr:wSer.chgOut||[],   type:'flow'},
    만기환수:{arr:wSer.maturity||[], type:'flow'},
    순증:    {arr:wSer.netAdd||[],   type:'flow'},
  });
  const annualCapaW = buildAnnualRows(a.months,{
    CAPA합계:  {arr:wSer.capa||[],          type:'flow'},
    신규:      {arr:wSer.newSubs||[],        type:'flow'},
    기변:      {arr:wSer.chgSubs||[],        type:'flow'},
    소매:      {arr:(d.channelSeries||{})['소매']||[],    type:'flow'},
    도매:      {arr:(d.channelSeries||{})['도매']||[],    type:'flow'},
    디지털:    {arr:(d.channelSeries||{})['디지털']||[],  type:'flow'},
    소상공인:  {arr:(d.channelSeries||{})['소상공인']||[],type:'flow'},
    B2B:       {arr:(d.channelSeries||{})['B2B']||[],    type:'flow'},
    ARPU:      {arr:(d.arpu||{}).series||[], type:'avg'},
  });

  // ── 유선 연 마감 테이블 ──────────────────────────────────────────
  const vSer = (d.wired||{}).series||{};
  const annualWired = buildAnnualRows(a.months,{
    유지:    {arr:vSer.total||[],      type:'stock'},
    인터넷:  {arr:vSer.internet||[],   type:'stock'},
    TV:      {arr:vSer.tv||[],         type:'stock'},
    신규:    {arr:vSer.newSubs||[],    type:'flow'},
    해지:    {arr:vSer.cancelSubs||[], type:'flow'},
    만기환수:{arr:vSer.maturity||[],   type:'flow'},
    영업순증:{arr:vSer.netAdd||[],     type:'flow'},
    순증:    {arr:vSer.netPure||[],    type:'flow'},
  });
  const wChNA = (d.wired||{}).channelNetAdd||DEFAULT_SUBSCRIBER_DATA.wired.channelNetAdd||{};
  const CH_V=['소매','소상공인','도매','디지털'];
  const annualCapaV = buildAnnualRows(a.months, Object.fromEntries([
    ['I+TV합계',{arr:CH_V.reduce((acc,ch)=>{const a2=(wChNA.itv||{})[ch]||[];a2.forEach((v,i)=>acc[i]=(acc[i]||0)+v);return acc;},[]),type:'flow'}],
    ...CH_V.map(ch=>([ch,{arr:(wChNA.itv||{})[ch]||[],type:'flow'}]))
  ]));

  // 연 마감 테이블 HTML 생성
  const annualTableW_sub = renderAnnualTable(annualWireless,[
    {label:'연도',align:'left',f:r=>`<b>${r.year}</b>${r._months<12?`<span style="font-size:9px;color:#94a3b8;margin-left:4px">(${r._months}개월)</span>`:''}` },
    {label:'유지(말)',   f:r=>fmtSubNum(r.유지)},
    {label:'CAPA',      f:r=>fmtSubNum(r.CAPA)},
    {label:'월평균CAPA',f:r=>`<span style="color:#93c5fd">${fmtSubNum(Math.round(r.CAPA/r._months))}</span>`},
    {label:'신규',      f:r=>fmtSubNum(r.신규)},
    {label:'기변',      f:r=>fmtSubNum(r.기변)},
    {label:'해지',      f:r=>fmtSubNum(r.해지)},
    {label:'기변OUT',   f:r=>fmtSubNum(r.기변OUT)},
    {label:'만기환수',  f:r=>fmtSubNum(r.만기환수)},
    {label:'순증',      f:r=>`<span style="color:${r.순증>=0?'#86efac':'#fca5a5'};font-weight:700">${fmtSubDiff(r.순증)}</span>`},
    {label:'월평균순증',f:r=>`<span style="color:${r.순증>=0?'#86efac':'#fca5a5'}">${fmtSubDiff(Math.round(r.순증/r._months))}</span>`},
  ]);
  const annualTableW_capa = renderAnnualTable(annualCapaW,[
    {label:'연도',align:'left',f:r=>`<b>${r.year}</b>${r._months<12?`<span style="font-size:9px;color:#94a3b8;margin-left:4px">(${r._months}개월)</span>`:''}` },
    {label:'CAPA합계',  f:r=>fmtSubNum(r.CAPA합계)},
    {label:'월평균CAPA',f:r=>`<span style="color:#93c5fd">${fmtSubNum(Math.round(r.CAPA합계/r._months))}</span>`},
    {label:'신규',      f:r=>fmtSubNum(r.신규)},
    {label:'기변',      f:r=>fmtSubNum(r.기변)},
    {label:'소매',      f:r=>fmtSubNum(r.소매)},
    {label:'도매',      f:r=>fmtSubNum(r.도매)},
    {label:'디지털',    f:r=>fmtSubNum(r.디지털)},
    {label:'소상공인',  f:r=>fmtSubNum(r.소상공인)},
    {label:'B2B',       f:r=>fmtSubNum(r.B2B)},
    {label:'평균ARPU',  f:r=>fmtWon(r.ARPU)},
  ]);
  const annualTableV_sub = renderAnnualTable(annualWired,[
    {label:'연도',align:'left',f:r=>`<b>${r.year}</b>${r._months<12?`<span style="font-size:9px;color:#94a3b8;margin-left:4px">(${r._months}개월)</span>`:''}` },
    {label:'유지(말)',   f:r=>fmtSubNum(r.유지)},
    {label:'인터넷(말)', f:r=>fmtSubNum(r.인터넷)},
    {label:'TV(말)',     f:r=>fmtSubNum(r.TV)},
    {label:'신규',       f:r=>fmtSubNum(r.신규)},
    {label:'월평균신규', f:r=>`<span style="color:#93c5fd">${fmtSubNum(Math.round(r.신규/r._months))}</span>`},
    {label:'해지',       f:r=>fmtSubNum(r.해지)},
    {label:'만기환수',   f:r=>fmtSubNum(r.만기환수)},
    {label:'영업순증',   f:r=>`<span style="color:${r.영업순증>=0?'#86efac':'#fca5a5'};font-weight:700">${fmtSubDiff(r.영업순증)}</span>`},
    {label:'순증',       f:r=>`<span style="color:${r.순증>=0?'#86efac':'#fca5a5'};font-weight:700">${fmtSubDiff(r.순증)}</span>`},
    {label:'월평균순증', f:r=>`<span style="color:${r.순증>=0?'#86efac':'#fca5a5'}">${fmtSubDiff(Math.round(r.순증/r._months))}</span>`},
  ]);
  const annualTableV_capa = renderAnnualTable(annualCapaV,[
    {label:'연도',align:'left',f:r=>`<b>${r.year}</b>${r._months<12?`<span style="font-size:9px;color:#94a3b8;margin-left:4px">(${r._months}개월)</span>`:''}` },
    {label:'I+TV합계',   f:r=>fmtSubDiff(r['I+TV합계'])},
    {label:'월평균',     f:r=>`<span style="color:#93c5fd">${fmtSubDiff(Math.round(r['I+TV합계']/r._months))}</span>`},
    ...CH_V.map(ch=>({label:ch, f:r=>fmtSubDiff(r[ch])})),
  ]);
  // ── 2024년 행 prepend (팩트북에서 '24년계 컬럼 읽은 값) ──────────
  // ── 2024년 연간 확정값 fallback (마감 완료, 팩트북 미업로드 시 사용) ──
  const Y2024_W_FB = {total:1822479, capa:618677, newSubs:225132, chgSubs:357505, churn:206391, chgOut:396941, maturity:104174, netAdd:18741, chSeries:{소매:203866,도매:240483,디지털:138022,소상공인:0,B2B:33166}, arpu:65661};
  const Y2024_V_FB = {total:362563, internet:204798, tv:157765, newSubs:92868, cancelSubs:47015, maturity:92090, netAdd:45853, netPure:-46237, chItv:{소매:33874,소상공인:12391,도매:31901,디지털:598}};
  const y24w = (d.y2024&&d.y2024.wireless&&d.y2024.wireless.total) ? d.y2024.wireless : Y2024_W_FB;
  const y24v = (d.y2024&&d.y2024.wired&&d.y2024.wired.total)    ? d.y2024.wired    : Y2024_V_FB;
  if(y24w.total){
    annualWireless.unshift({year:'2024년',_months:12, 유지:y24w.total, CAPA:y24w.capa, 신규:y24w.newSubs, 기변:y24w.chgSubs, 해지:y24w.churn, 기변OUT:y24w.chgOut, 만기환수:y24w.maturity, 순증:y24w.netAdd});
    annualCapaW.unshift({year:'2024년',_months:12, CAPA합계:y24w.capa, 신규:y24w.newSubs, 기변:y24w.chgSubs, 소매:(y24w.chSeries||{}).소매||0, 도매:(y24w.chSeries||{}).도매||0, 디지털:(y24w.chSeries||{}).디지털||0, 소상공인:(y24w.chSeries||{}).소상공인||0, B2B:(y24w.chSeries||{}).B2B||0, ARPU:Math.round(y24w.arpu||0)});
  }
  if(y24v.total){
    annualWired.unshift({year:'2024년',_months:12, 유지:y24v.total, 인터넷:y24v.internet, TV:y24v.tv, 신규:y24v.newSubs, 해지:y24v.cancelSubs, 만기환수:y24v.maturity, 영업순증:y24v.netAdd, 순증:y24v.netPure});
    const ch24 = y24v.chItv||{};
    annualCapaV.unshift({year:'2024년',_months:12, 'I+TV합계':CH_V.reduce((s,k)=>s+(ch24[k]||0),0), 소매:ch24.소매||0, 소상공인:ch24.소상공인||0, 도매:ch24.도매||0, 디지털:ch24.디지털||0});
  }
  // ─────────────────────────────────────────────────────────────────
  const tabs={
    w_summary:`<div class="sub-grid"><div class="sub-card"><div class="sub-lbl">${lbl1}</div><div class="sub-kpi">${fmtSubNum(prevYearEndVal)}</div></div><div class="sub-card"><div class="sub-lbl">${lbl2}</div><div class="sub-kpi">${fmtSubNum(a.m251.endingSubs||0)}</div></div><div class="sub-card"><div class="sub-lbl">${lbl3}</div><div class="sub-kpi">${fmtSubNum(endCur)}</div></div><div class="sub-card"><div class="sub-lbl">${lbl4}</div><div class="sub-kpi">${yoy>=0?'+':''}${yoy.toFixed(2)}%</div></div></div><div class="sub-block"><h3>순증 검증</h3>${renderSubscriberTable([{k:'순증 참조값(Factbook 순증행)',v:netRef},{k:'월말 가입자 증감(전월대비)',v:netByDelta},{k:'산식 계산값(CAPA-해지-기변OUT-만기환수)',v:netCalc}],[{label:'검증항목',align:'left',f:r=>r.k},{label:'값',f:r=>fmtSubNum(r.v)}])}<div class="sub-note" style="color:${(netRef===netByDelta&&netRef===netCalc)?'#86efac':'#fca5a5'}">${(netRef===netByDelta&&netRef===netCalc)?'검증 일치':'검증 불일치(원천 확인 필요)'}</div></div><div class="sub-block"><h3>순증 요인 분해</h3><div class="sub-mini" style="grid-template-columns:repeat(3,1fr)"><div class="sub-mini-item"><span class="sub-lbl">신규</span><b>${fmtSubNum(totalNew)}</b></div><div class="sub-mini-item"><span class="sub-lbl">기변</span><b>${fmtSubNum(totalChg)}</b></div><div class="sub-mini-item"><span class="sub-lbl">해지</span><b>${fmtSubNum(totalCancel)}</b></div><div class="sub-mini-item"><span class="sub-lbl">기변OUT</span><b>${fmtSubNum(totalChgOut)}</b></div><div class="sub-mini-item"><span class="sub-lbl">만기환수</span><b>${fmtSubNum(totalMaturity)}</b></div><div class="sub-mini-item"><span class="sub-lbl">CAPA</span><b>${fmtSubNum(capaCalc)}</b></div></div></div><div class="sub-block"><h3>📅 연 마감별 무선 가입자 현황</h3><div class="sub-note" style="margin-bottom:8px">※ 유지=연말 재고, CAPA·신규·기변·해지·순증=해당 연도 합계 / 미마감 연도는 월 수 표시</div>${annualTableW_sub}</div>`,
    w_capa:`<div class="sub-grid" style="grid-template-columns:repeat(2,1fr)"><div class="sub-card"><div class="sub-lbl">무선 CAPA(일반후불)</div><div class="sub-kpi">${fmtSubNum(wCapa)}</div><div style="font-size:11px;color:${wCapa-wCapaPrev>=0?'#86efac':'#fca5a5'}">전월대비 ${fmtSubDiff(wCapa-wCapaPrev)}</div></div><div class="sub-card"><div class="sub-lbl">무선 ARPU</div><div class="sub-kpi">${fmtWon(arpuCur)}</div><div style="font-size:11px;color:${arpuDelta>=0?'#86efac':'#fca5a5'}">전월대비 ${(arpuDelta>=0?'+':'')+Math.round(arpuDelta).toLocaleString('ko-KR')}원</div></div></div><div class="sub-block"><h3>무선 채널별 CAPA/기여</h3>${renderSubscriberTable(channelRows,[{label:'채널',align:'left',f:r=>r.channel},{label:'CAPA',f:r=>fmtSubNum(r.capa)},{label:'전월대비',f:r=>fmtSubDiff(r.delta)},{label:'비중',f:r=>r.share.toFixed(1)+'%'}])}</div><div class="sub-block"><h3>📅 연 마감별 무선 CAPA 현황</h3><div class="sub-note" style="margin-bottom:8px">※ 모든 항목 해당 연도 합계 / 평균ARPU=월 평균 / 미마감 연도는 월 수 표시</div>${annualTableW_capa}</div>`,
    ...
    v_summary:(()=>{
      ...
      <div class="sub-block"><h3>📅 연 마감별 유선 가입자 현황</h3><div class="sub-note" style="margin-bottom:8px">※ 유지·인터넷·TV=연말 재고, 신규·해지·순증=해당 연도 합계 / 미마감 연도는 월 수 표시</div>${annualTableV_sub}</div>`;
    })(),
    v_capa:(()=>{
      const ws = d.wired||{};
      const wSeries = ws.series||{};
  ...
  // 월 선택 드롭다운 — 팩트북 보유 월 목록
  const monthOpts = a.months.map((m,i)=>{
    const ml = m.replace(/'/g,'').trim();
    return `<option value="${i}" ${i===a.idx?'selected':''}>${ml}</option>`;
  }).join('');
  const monthSelector = `<select style="background:#1e293b;color:#e2e8f0;border:1px solid #334155;border-radius:6px;padding:4px 8px;font-size:12px;cursor:pointer" onchange="window.__subMonthIdx=parseInt(this.value);initSubscriberUI()">
    <option value="" disabled>월 선택</option>${monthOpts}
  </select>`;

  document.getElementById('subscriberPanel').innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px">
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        <div class="sub-chip">기준월 ${a.currentMonth}</div>
        <div class="sub-chip alt">원천: ${d.sourceFile==='업로드 Factbook'?'업로드 Factbook':'기본값'}</div>
        <div class="sub-chip warn">순증 검증 포함</div>
      </div>
      ${a.months.length>1?`<div style="display:flex;align-items:center;gap:6px"><span style="font-size:11px;color:#64748b">보유월:</span>${monthSelector}</div>`:''}
    </div>
...
function parseFactbookExcel(wb){
  try{
    ...
    extracted._dataVersion = DEFAULT_SUBSCRIBER_DATA._dataVersion||0; // 새로고침 후 복원 허용
    try{
      Object.keys(subscriberData).forEach(function(k){ delete subscriberData[k]; });
      Object.assign(subscriberData, extracted);
    }catch(ex){ subscriberData = extracted; }
    saveSubscriberData();
    initSubscriberUI();
    initDashboard();
    showUpStatus('factbook','ok','✅ 가입자 데이터 업데이트 완료\n기준월: '+extracted.baseMonth+'\n시트: '+(extracted.sheetNames||[]).join(', '));
    showToast(
      `Factbook 업로드 완료<br><span style="font-weight:400;font-size:12px">기준월 <b>${extracted.baseMonth}</b> · 가입자 탭이 업데이트되었습니다</span>`,
      'ok', 4000
    );
  }catch(err){
...
function extractMonthFromFilename(filename){
  ...
  // 패턴0: "1_2월","1~2월","1 2월" 범위 표기 → 마지막 월 추출 (예: 26년 1Q 1 2월 → 2026.02)
  const m0 = fn.match(/(?:20)?(\d{2})년.*?(\d{1,2})\s*[~]?\s*(\d{1,2})월/);
  if(m0 && parseInt(m0[3]) > parseInt(m0[2])){
    const yy=parseInt(m0[1]); const mm=parseInt(m0[3]);
    return `${yy<50?2000+yy:1900+yy}.${String(mm).padStart(2,'0')}`;
  }
...
function parseTaskExcel(wb, filename){
  const month = extractMonthFromFilename(filename);
  if(month) setGlobalBaseMonth(month, '사업과제');
  ...
  if(tasks.length>0){D.tasks=tasks;initTaskUI();initDashboard();showUpStatus('task','ok','✅ '+tasks.length+'개 과제 로드!'+(month?` · 기준월 ${month}`:''));autoSaveCurrent('tasks');}
...
function parseProfitExcel(wb, filename){try{
  ...
  // 공헌이익 = 영업이익 행 기준 (점프업 조정 없음 - 규칙①)
  var contribSrc=findInc('공헌이익')||find('영업이익');
  if(contribSrc){ D.profit.contribution=mk(contribSrc); }
  ...
  // ── 유통플랫폼단 시트 파싱 ──
  var pfSheet=wb.Sheets['유통플랫폼단'];
  if(pfSheet){
    var pfRows=XLSX.utils.sheet_to_json(pfSheet,{header:1,defval:0});
    var pfNorm=function(x){return String(x||'').trim().replace(/\s+/g,'');};
    var pfFind=function(l){return pfRows.find(function(r){return r[0]&&pfNorm(r[0])===pfNorm(l);});};
    var pfOpRow=pfFind('손익');var pfRevRow=pfFind('매출');
    var pfGpRow=pfFind('매출총이익');var pfSgaRow=pfFind('판매비와일반관리비');
    if(pfRevRow&&pfOpRow){
      // 열: 1=합계, 2=신사업, 3=유통플랫폼사업단(합산행 스킵), 4=플랫폼1, 5=플랫폼2, 6=모바일
      var pfV=function(row,col){return Math.round(Number(row&&row[col]||0));};
      // 유통플랫폼 영업이익(op) = 종합 시트 "영업이익(채널+전사공통비 포함)" 행의 유통플랫폼 컬럼(col12)
      var pfOpAdjRow=rows.find(function(r){return r[0]&&norm(r[0]).indexOf('채널+전사공통비')>=0;});
      var pfOpAdj=pfOpAdjRow?Math.round(Number(pfOpAdjRow[colIdx.small_biz+1]||0)):pfV(pfOpRow,1);
      D.profit.platform={
        note:'공헌이익=영업이익(전사공통비 배부 전) / 영업이익=전사공통비 포함 후',
        total:{revenue:pfV(pfRevRow,1),contribution:pfV(pfOpRow,1),sga:pfV(pfSgaRow,1),op:pfOpAdj},
        teams:[
          {nm:'신사업팀',revenue:pfV(pfRevRow,2),contribution:pfV(pfOpRow,2),sga:pfV(pfSgaRow,2),op:pfV(pfOpRow,2)},
          {nm:'플랫폼사업1팀',revenue:pfV(pfRevRow,4),contribution:pfV(pfOpRow,4),sga:pfV(pfSgaRow,4),op:pfV(pfOpRow,4)},
          {nm:'플랫폼사업2팀',revenue:pfV(pfRevRow,5),contribution:pfV(pfOpRow,5),sga:pfV(pfSgaRow,5),op:pfV(pfOpRow,5)},
          {nm:'모바일파트',revenue:pfV(pfRevRow,6),contribution:pfV(pfOpRow,6),sga:pfV(pfSgaRow,6),op:pfV(pfOpRow,6)}
        ]
      };
    }
  }
  // ── 수수료 데이터 갱신 (D.commission.jan26) ──
  try{
    var feeMap={
      '수수료수입':        'total_fee',
      '수수료수입-KT(RDS)':'rds',
    ...
    var feeData={};
    Object.keys(feeMap).forEach(function(label){
      var row=find(label);
      if(!row) return;
      var key=feeMap[label];
      feeData[key]={
        total:Math.round(row[1]||0)/1e8,
        retail:Math.round(row[colIdx.retail]||0)/1e8,
        wholesale:Math.round(row[colIdx.wholesale]||0)/1e8,
        digital:Math.round(row[colIdx.digital]||0)/1e8,
        enterprise:Math.round(row[colIdx.enterprise]||0)/1e8,
        iot:Math.round(row[colIdx.iot]||0)/1e8,
        corporate_sales:Math.round(row[colIdx.corporate_sales]||0)/1e8,
        small_biz:Math.round(row[colIdx.small_biz]||0)/1e8
      };
    });
    if(feeData.total_fee){
      var j26=D.commission.jan26;
      // 채널 합계 계산
      var tChanSum=+(feeData.total_fee.retail+feeData.total_fee.wholesale+feeData.total_fee.digital+
        feeData.total_fee.enterprise+feeData.total_fee.iot+feeData.total_fee.corporate_sales+feeData.total_fee.small_biz).toFixed(2);
      var tCommon=+((feeData.total_fee.total-tChanSum)||0).toFixed(2);
      // 수수료수입 합계
      j26.total_fee={
        total:+(feeData.total_fee.total.toFixed(2)),
        channel_subtotal:tChanSum,
        common_corp:+((tCommon*0.7)||0).toFixed(2),  // 공통비 추정 (전사:채널=7:3)
        common_ch:+((tCommon*0.3)||0).toFixed(2),
        retail:+feeData.total_fee.retail.toFixed(2),wholesale:+feeData.total_fee.wholesale.toFixed(2),
        digital:+feeData.total_fee.digital.toFixed(2),enterprise:+feeData.total_fee.enterprise.toFixed(2),
        iot:+feeData.total_fee.iot.toFixed(2),corporate_sales:+feeData.total_fee.corporate_sales.toFixed(2),
        small_biz:+feeData.total_fee.small_biz.toFixed(2)
      };
      // 관리수수료
      if(feeData.mgmt){
        var mTotal=+feeData.mgmt.total.toFixed(2);
        var mChanSum=+(feeData.mgmt.retail+feeData.mgmt.wholesale+feeData.mgmt.digital+
          feeData.mgmt.enterprise+feeData.mgmt.iot+feeData.mgmt.corporate_sales+feeData.mgmt.small_biz).toFixed(2);
        j26.mgmt_fee={
          total:mTotal, channel_subtotal:mChanSum||mTotal, common:+((mTotal-mChanSum)||0).toFixed(2),
          retail:+feeData.mgmt.retail.toFixed(2), wholesale:+feeData.mgmt.wholesale.toFixed(2),
          digital:+feeData.mgmt.digital.toFixed(2), enterprise:+feeData.mgmt.enterprise.toFixed(2),
          iot:+feeData.mgmt.iot.toFixed(2), corporate_sales:+feeData.mgmt.corporate_sales.toFixed(2),
          small_biz:+feeData.mgmt.small_biz.toFixed(2)
        };
      }
      // 무선 정책수수료 (KT-RDS + 정책 합산)
      var rds=feeData.rds||{};var pol=feeData.policy||{};
      j26.policy_fee={
        total:+((rds.total||0)+(pol.total||0)).toFixed(2),
        rds:+(rds.total||0).toFixed(2),
        policy:+(pol.total||0).toFixed(2),
        retail:+((rds.retail||0)+(pol.retail||0)).toFixed(2),
        wholesale:+((rds.wholesale||0)+(pol.wholesale||0)).toFixed(2),
        digital:+((rds.digital||0)+(pol.digital||0)).toFixed(2),
        enterprise:+((rds.enterprise||0)+(pol.enterprise||0)).toFixed(2),
        iot:+((rds.iot||0)+(pol.iot||0)).toFixed(2),
        corporate_sales:+((rds.corporate_sales||0)+(pol.corporate_sales||0)).toFixed(2),
        small_biz:+((rds.small_biz||0)+(pol.small_biz||0)).toFixed(2),
        note:'KT-RDS 수수료 + 수수료수입 정책 합산 기준'
      };
      // fee_items 갱신
      var itemMap=[
        {nm:'KT(RDS)',k:'rds'},{nm:'관리',k:'mgmt'},{nm:'용역',k:'yongyek'},
        {nm:'유선',k:'wired'},{nm:'정책',k:'policy'},{nm:'P3.0',k:'p30'},
        {nm:'기타',k:'etc'},{nm:'매장성',k:'store'},{nm:'순액처리',k:'net_adj'}
      ];
      j26.fee_items=itemMap.filter(function(x){return feeData[x.k];}).map(function(x){
        var d=feeData[x.k];
        return {nm:x.nm,total:+d.total.toFixed(2),
          retail:+d.retail.toFixed(2),wholesale:+d.wholesale.toFixed(2),
          digital:+d.digital.toFixed(2),enterprise:+d.enterprise.toFixed(2),
          iot:+d.iot.toFixed(2),corporate_sales:+d.corporate_sales.toFixed(2),
          small_biz:+d.small_biz.toFixed(2)};
      });
    }
  }catch(feeErr){ warnings.push('⚠️ 수수료 데이터 파싱 오류: '+feeErr.message); }
...
function parseKpiExcel(wb, filename){try{
  ...
  // ── 헬퍼: 시트→rows: 모든 시트 순회해서 키워드 포함 찾기 ──
  const _allRows = {};
  wb.SheetNames.forEach(function(sn){
    _allRows[sn] = XLSX.utils.sheet_to_json(wb.Sheets[sn],{header:1,defval:0});
  });
  const sheetRows = function(name){
    // 1) 직접 매칭
    if(_allRows[name]) return _allRows[name];
    // 2) 공백·특수문자 제거 후 매칭
    const clean = function(s){ return String(s||'').replace(/[\s\(\)]/g,''); };
    const kw = clean(name);
    for(var i=0;i<wb.SheetNames.length;i++){
      if(clean(wb.SheetNames[i])===kw) return _allRows[wb.SheetNames[i]];
    }
    // 3) 키워드 포함 매칭 (예: '종합' 포함 시트)
    var keyword = name.replace(/^1Q\s*/,'').trim();
    for(var j=0;j<wb.SheetNames.length;j++){
      if(wb.SheetNames[j].indexOf(keyword)>=0) return _allRows[wb.SheetNames[j]];
    }
    return [];
  };

  // ── 헬퍼: 셀값 정규화 ──
  const hqName = function(v){ return String(v||'').trim(); };
  // ── 헬퍼: 숫자 변환 ──
  const toNum = function(v){ var n=parseFloat(v); return isNaN(n)?null:n; };
  // ── 헬퍼: rows에서 컬럼 오프셋 자동 감지 (시트마다 독립) ──
  // 원칙: 이름열 다음 열이 50~120 KPI 점수 + 다음 열이 1~9 순위여야 본부행
  const detectC = function(rows){
    for(var i=0;i<rows.length;i++){
      var r=rows[i];
      // r[0]이 실제 문자열(2글자 이상, 공백없음)이고, r[1]=점수, r[2]=순위인 경우만 C=0
      var r0=r[0]; var r1=r[1]; var r2=r[2];
      if(typeof r0==='string' && r0.length>=2 && r0.indexOf(' ')<0 && r0!=='0'){
        var s0=toNum(r1), rk0=toNum(r2);
        if(s0!==null&&s0>=50&&s0<=110&&rk0!==null&&rk0>=1&&rk0<=9) return 0;
      }
      // r[1]이 실제 문자열이고, r[2]=점수, r[3]=순위인 경우만 C=1
      if(typeof r1==='string' && r1.length>=2 && r1.indexOf(' ')<0 && r1!=='0'){
        var s1=toNum(r2), rk1=toNum(r[3]);
        if(s1!==null&&s1>=50&&s1<=110&&rk1!==null&&rk1>=1&&rk1<=9) return 1;
      }
    }
    return 1; // 기본값
  };
  ...
  // ══ 1. 1Q 종합 → D.kpi ══
  // rowsG 먼저 로드 후 컬럼 오프셋 감지
  const rowsG = sheetRows('1Q 종합');
  // ── 컬럼 오프셋 자동 감지: r[0]=본부명이면 C=0, r[1]=본부명이면 C=1 ──
  const C = detectC(rowsG);
  // ── 헬퍼: 본부행 판별 ──
  const isHqRow = function(r){
    var nm = hqName(r[C]);
    if(!nm || nm.length<2 || nm.indexOf(' ')>=0) return false;
    var sc = toNum(r[C+1]); var rk = toNum(r[C+2]);
    if(sc===null || sc<40 || sc>120) return false;
    if(rk===null || rk<1 || rk>9) return false;
    return true;
  };
  // ── 헬퍼: 지사행 판별 ──
  const isJisaRow = function(r,suffix){
    var nm = hqName(r[C]);
    if(!nm || nm.length<2) return false;
    var sc = toNum(r[C+1]);
    if(sc===null || sc<40 || sc>120) return false;
    if(suffix) return nm.slice(-suffix.length)===suffix;
    var rk=toNum(r[C+2]); return rk!==null && rk>=1;
  };
  // 디버그: rows[12]의 실제 값과 타입 확인
  const dbgRow = rowsG[12]||rowsG[11]||rowsG[10]||[];
  const dbgMsg = 'rows총:'+rowsG.length+' | row12: r0='+JSON.stringify(dbgRow[0])+' r1='+JSON.stringify(dbgRow[1])+' r2='+JSON.stringify(dbgRow[2])+'('+typeof dbgRow[2]+') r3='+JSON.stringify(dbgRow[3])+'('+typeof dbgRow[3]+')';
  const C_dbg = 'C='+C+' | isHqRow(row12)='+isHqRow(dbgRow)+' nm='+JSON.stringify(hqName(dbgRow[C]))+' sc='+toNum(dbgRow[C+1])+' rk='+toNum(dbgRow[C+2]);
  console.log('[KPI DEBUG]', dbgMsg, C_dbg);
  const sheetDbg = wb.SheetNames.slice(0,5).join(', ');
  const kpi=[];
  rowsG.forEach(r=>{
    if(!isHqRow(r)) return;
    kpi.push({hq:hqName(r[C]),ts:toNum(r[C+1])||0,rk:toNum(r[C+2])||0,
      rt:{t:toNum(r[C+3])||0,s:toNum(r[C+4])||0,p:toNum(r[C+5])||0},
      wh:{t:toNum(r[C+6])||0,s:toNum(r[C+7])||0,p:toNum(r[C+8])||0},
      sm:{t:toNum(r[C+9])||0,s:toNum(r[C+10])||0,p:toNum(r[C+11])||0}});
  });
  if(kpi.length<3){showUpStatus('kpi','err','❌ KPI 데이터 부족 (종합 '+kpi.length+'개 / 행'+rowsG.length+') 시트목록: '+wb.SheetNames.join(', ')+'\n디버그: '+dbgMsg+' '+C_dbg);return;}
  kpi.sort((a,b)=>b.ts-a.ts);kpi.forEach((r,i)=>r.rk=i+1);
  D.kpi=kpi;

  // ══ 2. 1Q 소매 → D.kpi_retail_detail + kpiBranch.retail ══
  const rowsR = sheetRows('1Q 소매');
  const retDetail={};
  const retBranch=[];
  const RC=detectC(rowsR); // 소매 시트 독립 C 감지
  rowsR.forEach(r=>{
    const nm=hqName(r[RC]);
    if(!nm||nm.length<2||nm.indexOf(' ')>=0) return;
    const sc=toNum(r[RC+1])||0; const rk=toNum(r[RC+2])||0;
    if(sc>=40&&sc<=120&&rk>=1&&rk<=9&&sc>0){
      retDetail[nm]={total:sc,rank:rk,
        hubul:toNum(r[RC+4])||0,mnp:toNum(r[RC+5])||0,wired:toNum(r[RC+6])||0,mit:toNum(r[RC+7])||0,
        apru:toNum(r[RC+8])||0,strategy:toNum(r[RC+9])||0,rvoc:toNum(r[RC+10])||0,
        oneg:toNum(r[RC+11])||0,tcsi:toNum(r[RC+12])||0,
        policy_rev:toNum(r[RC+14])||0,store_profit:toNum(r[RC+15])||0};
    } else if(sc>=40&&sc<=110&&rk>=1&&rk<=20&&sc>0){
      retBranch.push({nm,sc,rk});
    }
  });
  if(Object.keys(retDetail).length>=3) D.kpi_retail_detail=retDetail;
  if(retBranch.length>=3){ retBranch.sort((a,b)=>b.sc-a.sc); retBranch.forEach((x,i)=>x.rk=i+1); D.kpiBranch=D.kpiBranch||{}; D.kpiBranch.retail=retBranch; }

  // ══ 3. 1Q 도매 → D.kpi_wholesale_detail + kpiBranch.wholesale ══
  const rowsW = sheetRows('1Q 도매');
  const whlDetail={};
  const whlBranch=[];
  const WC=detectC(rowsW); // 도매 시트 독립 C 감지
  rowsW.forEach(r=>{
    const nm=hqName(r[WC]);
    if(!nm||nm.length<2||nm.indexOf(' ')>=0) return;
    const sc=toNum(r[WC+1])||0; const rk=toNum(r[WC+2])||0;
    if(sc>=40&&sc<=120&&rk>=1&&rk<=9&&sc>0){
      whlDetail[nm]={total:sc,rank:rk,
        hubul:toNum(r[WC+4])||0,mnp:toNum(r[WC+5])||0,wired:toNum(r[WC+6])||0,mit:toNum(r[WC+7])||0,
        rvoc:toNum(r[WC+8])||0,oneg:toNum(r[WC+9])||0,apru:toNum(r[WC+10])||0,
        infra:toNum(r[WC+11])||0,add_rev:toNum(r[WC+13])||0};
    } else if(sc>=40&&sc<=120&&rk>=1&&rk<=20&&sc>0){
      whlBranch.push({nm,sc,rk});
    }
  });
  if(Object.keys(whlDetail).length>=3) D.kpi_wholesale_detail=whlDetail;
  if(whlBranch.length>=3){ whlBranch.sort((a,b)=>b.sc-a.sc); whlBranch.forEach((x,i)=>x.rk=i+1); D.kpiBranch=D.kpiBranch||{}; D.kpiBranch.wholesale=whlBranch; }

  // ══ 4. 1Q 소상공인 → D.kpi_smb_detail (헤더가 중간에 있으므로 본부 찾은 후 읽기) ══
  const rowsS = sheetRows('1Q 소상공인');
  const smbDetail={};
  const SC=detectC(rowsS); // 소상공인 시트 독립 C 감지
  rowsS.forEach(r=>{
    const nm=hqName(r[SC]);
    if(!nm||nm.length<2||nm.indexOf(' ')>=0) return;
    const sc=toNum(r[SC+1])||0; const rk=toNum(r[SC+2])||0;
    if(!(sc>=40&&sc<=120&&rk>=1&&rk<=9)) return;
    smbDetail[nm]={total:sc,rank:rk,
      hubul:toNum(r[SC+4])||0,wired:toNum(r[SC+5])||0,mit:toNum(r[SC+6])||0,
      strategy:toNum(r[SC+7])||0,rvoc:toNum(r[SC+8])||0,op_profit:toNum(r[SC+9])||0};
  });
  if(Object.keys(smbDetail).length>=3) D.kpi_smb_detail=smbDetail;

  // ══ 5. variance 재계산 ══
  const hqs=['강북본부','강남본부','강서본부','동부본부','서부본부'];
  function calcVar(obj,key){ const vs=hqs.map(h=>(obj[h]||{})[key]||0).filter(v=>v>0); return vs.length>1?Math.max(...vs)-Math.min(...vs):0; }
  D.variance={
    total: calcVar(Object.fromEntries(kpi.map(k=>[k.hq,{ts:k.ts}])),'ts'),
    retail: calcVar(retDetail,'total'), wholesale: calcVar(whlDetail,'total'), smb: calcVar(smbDetail,'total'),
    retail_sales: calcVar(Object.fromEntries(kpi.map(k=>[k.hq,{v:k.rt.s}])),'v'),
    retail_profit: calcVar(Object.fromEntries(kpi.map(k=>[k.hq,{v:k.rt.p}])),'v'),
    wh_sales: calcVar(Object.fromEntries(kpi.map(k=>[k.hq,{v:k.wh.s}])),'v'),
    wh_profit: calcVar(Object.fromEntries(kpi.map(k=>[k.hq,{v:k.wh.p}])),'v'),
    smb_sales: calcVar(Object.fromEntries(kpi.map(k=>[k.hq,{v:k.sm.s}])),'v'),
    smb_profit: calcVar(Object.fromEntries(kpi.map(k=>[k.hq,{v:k.sm.p}])),'v')
  };

  // ══ 6. UI 갱신 ══
  initKpiUI(); initDashboard();
  const det=`소매 ${Object.keys(retDetail).length}본부, 도매 ${Object.keys(whlDetail).length}본부, 소상공인 ${Object.keys(smbDetail).length}본부`;
  showUpStatus('kpi','ok',`✅ KPI 전체 업데이트!${month?` · ${month}`:''}\n${det} · 지사 소매 ${retBranch.length}개 / 도매 ${whlBranch.length}개`);
  autoSaveCurrent('kpi');
}catch(err){showUpStatus('kpi','err','❌ '+err.message+' ('+err.stack.split('\n')[1]+')');}
}
...
function showToast(msg, type, duration){
  ...
  if(!container){
    ...
    container.style.cssText = 'position:fixed;top:60px;left:50%;transform:translateX(-50%);z-index:99999;display:flex;flex-direction:column;align-items:center;gap:8px;pointer-events:none;width:90%;max-width:480px';
    document.body.appendChild(container);
  }
  const colors = {
    ok:   {bg:'#052e16', border:'#16a34a', icon:'✅', text:'#86efac'},
    err:  {bg:'#450a0a', border:'#dc2626', icon:'❌', text:'#fca5a5'},
    info: {bg:'#0c1a2e', border:'#3b82f6', icon:'ℹ️', text:'#93c5fd'},
  };
  const c = colors[type||'ok'];
  const toast = document.createElement('div');
  toast.style.cssText = `background:${c.bg};border:1.5px solid ${c.border};border-radius:12px;padding:14px 18px;box-shadow:0 8px 32px rgba(0,0,0,.5);pointer-events:auto;width:100%;animation:_toastIn .25s ease;`;
  toast.innerHTML = `<div style="display:flex;align-items:flex-start;gap:10px">
    <span style="font-size:18px;line-height:1.3">${c.icon}</span>
    <div style="flex:1;font-size:13px;color:${c.text};font-weight:600;line-height:1.6;word-break:keep-all">${msg}</div>
    <span style="font-size:16px;cursor:pointer;color:#64748b;padding:0 2px;line-height:1" onclick="this.parentElement.parentElement.remove()">×</span>
  </div>`;
  if(!document.getElementById('_toastStyle')){
    const s = document.createElement('style');
    s.id = '_toastStyle';
    s.textContent = '@keyframes _toastIn{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}@keyframes _toastOut{from{opacity:1}to{opacity:0;transform:translateY(-8px)}}';
    document.head.appendChild(s);
  }
  container.appendChild(toast);
  setTimeout(()=>{
    toast.style.animation='_toastOut .3s ease forwards';
    setTimeout(()=>toast.remove(), 300);
  }, duration);
}

// ==================== 월별 데이터 저장/로드 ====================
const MONTH_STORE_PREFIX = 'ktms_month_';
const MONTH_META_KEY = 'ktms_month_meta';
const TAB_ACTIVE_KEY = 'ktms_tab_active';   // {profit:'2026.02', kpi:'2026.01', ...}
const TAB_SNAP_PREFIX = 'ktms_tabsnap_';    // ktms_tabsnap_profit_2026.01

// 탭별로 저장/복원할 D 키 매핑
const TAB_DATA_KEYS = {
  profit:  ['profit','sga_detail','hq','hq_context','commission'], // commission도 BM파일에서 옴
  kpi:     ['kpi','kpi_retail_detail','kpi_wholesale_detail','kpi_smb_detail','kpiBranch','variance'],
  tasks:   ['tasks'],
  commission: ['commission'],
};
// 탭별 표시 이름
const TAB_LABEL = {profit:'BM손익',kpi:'KPI',tasks:'과제',commission:'수수료',subscriber:'가입자'};

function _getTabActive(){ try{return JSON.parse(localStorage.getItem(TAB_ACTIVE_KEY)||'{}');}catch(e){return{};} }
function _setTabActive(tab,month){ try{const m=_getTabActive();m[tab]=month;localStorage.setItem(TAB_ACTIVE_KEY,JSON.stringify(m));}catch(e){} }

function saveTabSnapshot(tab, month){
  if(!tab||!month) return;
  const keys = TAB_DATA_KEYS[tab]||[];
  const snap = {_tab:tab,_month:month,_savedAt:Date.now()};
  keys.forEach(k=>{ if(D[k]!==undefined) snap[k]=JSON.parse(JSON.stringify(D[k])); }); // 깊은 복사
  try{
    localStorage.setItem(TAB_SNAP_PREFIX+tab+'_'+month, JSON.stringify(snap));
    _setTabActive(tab, month); // 저장 성공 시에만 active 갱신
    console.log('[TabSnap] 저장 완료:', tab, month);
  }catch(e){
    console.error('[TabSnap] 저장 실패:', tab, month, e);
    // 용량 부족 시 오래된 항목 삭제 후 재시도
    try{
      const allKeys=Object.keys(localStorage).filter(k=>k.startsWith(TAB_SNAP_PREFIX));
      if(allKeys.length>20) localStorage.removeItem(allKeys[0]);
      localStorage.setItem(TAB_SNAP_PREFIX+tab+'_'+month, JSON.stringify(snap));
      _setTabActive(tab, month);
    }catch(e2){}
  }
}

function loadTabSnapshot(tab, month){
  try{ const r=localStorage.getItem(TAB_SNAP_PREFIX+tab+'_'+month); return r?JSON.parse(r):null; }catch(e){return null;}
}

function restoreTabSnapshot(tab, month){
  const snap = loadTabSnapshot(tab, month);
  if(!snap) return false;
  const keys = TAB_DATA_KEYS[tab]||[];
  keys.forEach(k=>{ if(snap[k]!==undefined) D[k]=snap[k]; });
  return true;
}

function getTabMonth(tab){ return (_getTabActive()[tab])||null; }

function updateHdrBadge(month){
  ['hdrMonthBadge','dashMonth2'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.textContent=month||'-';
  });
}

function _getMonthMeta(){
  try{ return JSON.parse(localStorage.getItem(MONTH_META_KEY)||'{}'); }catch(e){ return {}; }
}
function _setMonthMeta(meta){
  try{ localStorage.setItem(MONTH_META_KEY, JSON.stringify(meta)); }catch(e){}
}
...
function captureSnapshot(){
  return {
    ...
    kpi_smb_detail: D.kpi_smb_detail,
    kpiBranch: D.kpiBranch,
    variance: D.variance,
    hq_context: D.hq_context,
...
function restoreSnapshot(snap){
  if(!snap) return;
  const keys = ['period','baseMonth','tasks','profit','sga_detail','hq','kpi',
    'kpi_retail_detail','kpi_wholesale_detail','kpi_smb_detail','kpiBranch',
    'variance','hq_context','monthly25','commission'];
  keys.forEach(k=>{ if(snap[k]!==undefined) D[k]=snap[k]; });
}

function saveMonthToStorage(month){
  if(!month) return false;
  try{
    const snap = captureSnapshot();
    localStorage.setItem(MONTH_STORE_PREFIX+month, JSON.stringify(snap));
    const meta = _getMonthMeta();
    meta[month] = { savedAt: snap._savedAt, taskCount: (snap.tasks||[]).length };
    _setMonthMeta(meta);
    return true;
  }catch(e){ console.error('save failed', e); return false; }
}

function loadMonthFromStorage(month){
  try{
    const raw = localStorage.getItem(MONTH_STORE_PREFIX+month);
...
function deleteMonthFromStorage(month){
  try{
    localStorage.removeItem(MONTH_STORE_PREFIX+month);
    const meta = _getMonthMeta();
    delete meta[month];
    _setMonthMeta(meta);
  }catch(e){}
...
function saveCurrentMonth(showPicker){
  const month = D.baseMonth || '2026.01';
  const ok = saveMonthToStorage(month);
  if(ok){
    // 헤더 배지 반짝임 효과
    const badge = document.getElementById('hdrMonthBadge');
    if(badge){ badge.style.background='rgba(56,189,248,.4)'; setTimeout(()=>badge.style.background='',600); }
    // 피커가 열려 있으면 목록 갱신
    if(showPicker) showMonthPicker();
    else{
      const overlay = document.getElementById('monthPickerOverlay');
      if(overlay && !overlay.classList.contains('hidden')) renderMpSavedList();
    }
  }
}

function autoSaveCurrent(tab){
  const month = D.baseMonth||'2026.01';
  // 탭 스냅샷은 즉시(동기) 저장 — setTimeout 안에 두면 새로고침 시 유실됨
  if(tab) saveTabSnapshot(tab, month);
  updateHdrBadge(month);
  // 전역 전체 스냅샷은 비동기 (크기가 크므로)
  setTimeout(()=>{ saveMonthToStorage(month); }, 100);
}

function switchToMonth(month){
  const snap = loadMonthFromStorage(month);
  if(!snap){
    // 저장 안 된 월 클릭 시 안내
    const el = document.getElementById('mpMonthGrid');
    if(el){
      const btn = el.querySelector(`.mp-m[data-month="${month}"]`);
      if(btn){ btn.style.animation='none'; btn.offsetHeight; btn.style.animation='shake .3s'; }
    }
    return;
  }
  ...
  // 전체 탭 UI 재초기화
  try{ initTaskUI(); }catch(e){}
  try{ initProfitUI(); renderTrendChart(); }catch(e){}
  try{ initKpiUI(); }catch(e){}
  try{ initSubscriberUI(); }catch(e){}
  try{ initCommission(); }catch(e){}
  try{ initDashboard(); }catch(e){}
  closeMonthPicker();
  // 헤더 배지 반짝임
  const badge=document.getElementById('hdrMonthBadge');
  if(badge){ badge.style.background='rgba(56,189,248,.35)'; setTimeout(()=>badge.style.background='',700); }
}
...
function showMonthPicker(){
  renderMpMonthGrid();
  renderMpSavedList();
  document.getElementById('monthPickerOverlay').classList.remove('hidden');
}
function closeMonthPicker(){
  document.getElementById('monthPickerOverlay').classList.add('hidden');
}

function renderMpMonthGrid(){
  const stored = getStoredMonths();
  const currentMonth = D.baseMonth || '2026.01';
  const year = currentMonth.split('.')[0] || '2026';
  const months = Array.from({length:12},(_,i)=>`${year}.${String(i+1).padStart(2,'0')}`);
  const el = document.getElementById('mpMonthGrid');
  if(!el) return;
  el.innerHTML = months.map(m=>{
    const isActive = m === currentMonth;
    const hasData = stored.includes(m);
    const label = m.split('.')[1]+'월';
    const meta = _getMonthMeta()[m]||{};
    const savedTime = meta.savedAt ? new Date(meta.savedAt).toLocaleString('ko-KR',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}) : '';
    const tooltip = hasData ? `저장됨\n${savedTime}\n클릭하여 불러오기` : '저장된 데이터 없음';
    return `<div class="mp-m${isActive?' active':''}${hasData&&!isActive?' has-data':' no-data'}" data-month="${m}" onclick="switchToMonth('${m}')" title="${tooltip}">${label}${hasData&&!isActive?'<span class="mp-m-dot">▶</span>':''}</div>`;
  }).join('');
}

function renderMpSavedList(){
  const stored = getStoredMonths();
  const meta = _getMonthMeta();
  const el = document.getElementById('mpSavedList');
  if(!el) return;
  if(!stored.length){ el.innerHTML='<div style="text-align:center;color:var(--t3);font-size:11px;padding:8px">저장된 데이터 없음</div>'; return; }
  el.innerHTML = stored.reverse().map(m=>{
    const info = meta[m]||{};
    const dt = info.savedAt ? new Date(info.savedAt).toLocaleString('ko-KR',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}) : '';
    const tc = info.taskCount ? `과제 ${info.taskCount}개` : '';
    const isCur = m === (D.baseMonth||'2026.01');
    return `<div class="mp-saved-item">
      <div onclick="switchToMonth('${m}')" style="flex:1;cursor:pointer">
        <div class="mp-saved-month">${m}${isCur?' <span style="font-size:9px;color:var(--acc);font-weight:400">(현재)</span>':''}</div>
        <div class="mp-saved-meta">${[tc,dt].filter(Boolean).join(' · ')}</div>
      </div>
      <button onclick="switchToMonth('${m}')" style="font-size:10px;padding:3px 8px;background:rgba(56,189,248,.15);border:1px solid rgba(56,189,248,.3);border-radius:8px;color:var(--acc);cursor:pointer;margin-right:6px">불러오기</button>
      <span class="mp-saved-del" onclick="if(confirm('${m} 삭제?'))deleteMonthFromStorage('${m}'),renderMpMonthGrid(),renderMpSavedList()" title="삭제">🗑</span>
    </div>`;
  }).join('');
}
...
function exportDataJSON(){
  // 탭 스냅샷 머지
  const active = _getTabActive();
  ['profit','kpi','tasks','commission'].forEach(function(tab){
    const m = active[tab]; if(m) restoreTabSnapshot(tab, m);
  });
  normalizeProfitData();
  const exportMonth = D.baseMonth || active['profit'] || '2026.01';
  D.period = exportMonth.replace('.','년 ').replace(/^(\d+년 )0?(\d+)$/,'$1$2월');
  D.baseMonth = exportMonth;

  const snap = captureSnapshot();
  try{
    // ── subscriberData: localStorage 보다 in-memory 우선 (더 신뢰성 높음) ──
    const memSub = (subscriberData && subscriberData.months && subscriberData.months.length)
      ? subscriberData
      : null;
    const uploaded = localStorage.getItem('ktms_factbook_uploaded');
    const subRaw   = uploaded || localStorage.getItem('ktms_subscriber_data');
    const lsSub    = subRaw ? JSON.parse(subRaw) : null;
    // 메모리 데이터가 더 최신이면 우선 사용 (months 길이 비교)
    const bestSub  = (memSub && (!lsSub || (memSub.months||[]).length >= (lsSub.months||[]).length))
      ? memSub : lsSub;
    if(bestSub) snap._subscriberData = JSON.parse(JSON.stringify(bestSub)); // deep copy
    snap._factbookUploaded = null; // _subscriberData에 이미 포함됨
    snap._tabActive = active;
    snap._exportedAt = new Date().toISOString();
    snap._exportMonth = exportMonth;
    snap._subMonths = (bestSub && bestSub.months) ? bestSub.months.length : 0; // 디버그용
  }catch(e){ console.warn('exportDataJSON snap error', e); }

  const json = JSON.stringify(snap, null, 2);
  const blob = new Blob([json], {type:'application/json;charset=utf-8'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'KT_MS_데이터_' + exportMonth + '.json';
  a.click();
  setTimeout(()=>URL.revokeObjectURL(a.href), 1000);
  // 내보내기 완료 토스트
  const nMonths = snap._subMonths || 0;
  showToast(
    `JSON 내보내기 완료<br><span style="font-weight:400;font-size:12px">` +
    `기준월 <b>${exportMonth}</b> · 가입자 데이터 <b>${nMonths}개월</b> 포함</span>`,
    'ok', 4000
  );
}
...
function importDataJSON(input){
  ...
  if(!file) return;
  const statusEl = document.getElementById('sJsonImport');
  if(statusEl) statusEl.innerHTML = '<span style="color:#fbbf24">⏳ 불러오는 중...</span>';
  const reader = new FileReader();
  reader.onload = function(e){
    try{
      const snap = JSON.parse(e.target.result);

      // ① D 객체 복원 (손익·KPI·과제·수수료)
      restoreSnapshot(snap);
      normalizeProfitData();

      // ② subscriberData 복원 — 기존 객체 in-place 뮤테이션 (let 바인딩 대응)
      if(snap._subscriberData && snap._subscriberData.months && snap._subscriberData.months.length){
        // 기존 객체의 키를 모두 지우고 새 데이터로 덮어씀
        try{
          if(subscriberData && typeof subscriberData === 'object'){
            Object.keys(subscriberData).forEach(function(k){ try{delete subscriberData[k];}catch(ex){} });
            Object.assign(subscriberData, snap._subscriberData);
          } else {
            subscriberData = snap._subscriberData;
          }
        }catch(ex){
          subscriberData = snap._subscriberData;
        }
        // sourceFile 마킹 보장
        if(!subscriberData.sourceFile) subscriberData.sourceFile = '업로드 Factbook';
        // _dataVersion 주입
        try{ subscriberData._dataVersion = DEFAULT_SUBSCRIBER_DATA._dataVersion||0; }catch(ex){}
        // 저장
        saveSubscriberData();
        // factbookUploaded 별도 키도 명시 저장
        if(snap._factbookUploaded){
          try{
            const fbStr = typeof snap._factbookUploaded==='string'
              ? snap._factbookUploaded
              : JSON.stringify(snap._factbookUploaded);
            localStorage.setItem('ktms_factbook_uploaded', fbStr);
          }catch(ex){}
        }
      }

      // ③ 기준월 설정
      const m = snap._exportMonth || snap.baseMonth || D.baseMonth || '2026.01';
      D.baseMonth = m;
      window.__subMonthIdx = undefined; // 월 선택 초기화

      // ④ UI 전체 갱신
      updateHdrBadge(m);
      try{ initTaskUI(); }catch(ex){}
      try{ initProfitUI(); renderTrendChart(); }catch(ex){}
      try{ initKpiUI(); }catch(ex){}
      try{ initSubscriberUI(); }catch(ex){}
      try{ initCommission(); }catch(ex){}
      try{ initDashboard(); }catch(ex){}
      try{ initBriefing(); }catch(ex){}

      // 성공 토스트 — 화면 상단에 크게 표시
      const subMonth = subscriberData.baseMonth || '-';
      const subTotal = ((subscriberData.wireless||{}).total||0);
      showToast(
        `데이터 불러오기 완료<br>` +
        `<span style="font-weight:400;font-size:12px">` +
        `기준월 <b>${m}</b> · 가입자 <b>${subTotal ? subTotal.toLocaleString('ko-KR')+'명' : subMonth}</b> · ` +
        `모든 탭이 업데이트되었습니다</span>`,
        'ok', 5000
      );
      if(statusEl) statusEl.innerHTML = '<span style="color:#86efac">✅ 불러오기 완료 · 기준월: ' + m + '</span>';
    }catch(err){
      showToast('JSON 불러오기 실패<br><span style="font-weight:400;font-size:12px">' + err.message + '</span>', 'err', 6000);
      if(statusEl) statusEl.innerHTML = '<span style="color:#fca5a5">❌ 오류: ' + err.message + '</span>';
      console.error('importDataJSON error', err);
    }
    input.value = '';
  };
...
// ── 공유용 HTML 내보내기 ──
let _cachedAppJs = null;
let _cachedStyleCss = null;

async function _fetchSources(){
  if(!_cachedAppJs){
    try{
      const [appRes, cssRes] = await Promise.all([
        fetch('app.js').then(r=>r.text()),
        fetch('style.css').then(r=>r.text())
      ]);
      _cachedAppJs = appRes;
      _cachedStyleCss = cssRes;
    }catch(e){
      console.error('소스 fetch 실패', e);
    }
  }
}

async function exportShareableHTML(){
  const btn = document.querySelector('.mp-action-export, [onclick*="exportShareableHTML"]');
  const origText = btn ? btn.textContent : '';
  if(btn){ btn.textContent = '⏳ 생성 중...'; btn.disabled=true; }
  try{
    // ── 소스 파일 로드: fetch 우선, 실패 시 동기 XHR fallback (file:// 대응) ──
    function loadText(url){
      return fetch(url).then(r=>r.ok?r.text():Promise.reject()).catch(()=>
        new Promise((res,rej)=>{
          const xhr=new XMLHttpRequest();
          xhr.open('GET',url,false); // 동기
          try{ xhr.send(); res(xhr.status===0||xhr.status===200?xhr.responseText:null); }
          catch(e){ res(null); }
        })
      );
    }
    const [appJs, styleCss, dataJs, indexHtml] = await Promise.all([
      loadText('app.js'), loadText('style.css'),
      loadText('data.js'), loadText('index.html'),
    ]);
    if(!appJs||!styleCss||!dataJs||!indexHtml){
      alert('소스 파일 로드 실패: ' + [!appJs&&'app.js',!styleCss&&'style.css',!dataJs&&'data.js',!indexHtml&&'index.html'].filter(Boolean).join(', ') + '\n\n파일을 직접 열지 말고 폴더째로 열거나, 로컬 서버(Live Server 등)를 사용해 주세요.');
      return;
    }
    ...
    const active = _getTabActive();
    ['profit','kpi','tasks','commission'].forEach(function(tab){
      const month = active[tab]; if(month) restoreTabSnapshot(tab, month);
    });
    normalizeProfitData();
    const exportMonth = D.baseMonth || active['profit'] || '2026.01';
    D.period = exportMonth.replace('.','년 ').replace(/^(\d+년 )0?(\d+)$/,'$1$2월');
    ...
    // ── 스냅샷 JSON 생성 (subscriberData + factbook 포함) ──────────
    const snap = captureSnapshot();
    try{
      const uploaded = localStorage.getItem('ktms_factbook_uploaded');
      const subRaw   = uploaded || localStorage.getItem('ktms_subscriber_data');
      if(subRaw) snap._subscriberData = JSON.parse(subRaw);
      snap._factbookUploaded = uploaded || null;
      snap._tabActive = active;
    }catch(e){}
    const snapJson  = JSON.stringify(snap);
    const lsMeta    = JSON.stringify(_getMonthMeta());
    const lsTab     = JSON.stringify(_getTabActive());
    const fbUp      = localStorage.getItem('ktms_factbook_uploaded') || '';

    // 스냅샷 주입 스크립트
    const snapScript = '<scr'+'ipt>window.__KTMS_SNAPSHOT__='+snapJson+';<\/scr'+'ipt>';

    // localStorage 복원 스크립트 (새로고침 후에도 데이터 유지)
    const lsRestore = '<scr'+'ipt>try{'
      +'localStorage.setItem("'+MONTH_META_KEY+'",'+JSON.stringify(lsMeta)+');'
      +'localStorage.setItem("'+TAB_ACTIVE_KEY+'",'+JSON.stringify(lsTab)+');'
      +(fbUp?'localStorage.setItem("ktms_factbook_uploaded",'+JSON.stringify(fbUp)+');':'')
      +'}catch(e){}<\/scr'+'ipt>';

    // ── 외부 참조 → 완전 인라인 ───────────────────────────────────
    let html = indexHtml
      .replace(/<link[^>]*href="style\.css"[^>]*>/i, '<style>'+styleCss+'</style>')
      .replace(/<script[^>]*src="data\.js"[^>]*><\/script>/i, '<scr'+'ipt>'+dataJs+'<\/scr'+'ipt>'+snapScript)
      .replace(/<script[^>]*src="app\.js"[^>]*><\/script>/i,  '<scr'+'ipt>'+appJs+'<\/scr'+'ipt>')
      .replace(/<\/body>/i, lsRestore+'</body>');

    const blob = new Blob([html], {type:'text/html;charset=utf-8'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'KT_MS_경영관리_'+(D.baseMonth||'2026.01')+'_공유.html';
    a.click();
    setTimeout(()=>URL.revokeObjectURL(a.href), 1000);
  }catch(err){
    alert('내보내기 오류: '+err.message);
  }finally{
    if(btn){ btn.textContent = origText||'📤 공유용 HTML 내보내기'; btn.disabled=false; }
  }
}

// ── 앱 시작 시 스냅샷 주입 처리 ──
function _applyInjectedSnapshot(){
  if(window.__KTMS_SNAPSHOT__){
    restoreSnapshot(window.__KTMS_SNAPSHOT__);
    // subscriber + factbook 데이터 복원
    const snap = window.__KTMS_SNAPSHOT__;
    if(snap._subscriberData){
      try{
        const str = JSON.stringify(snap._subscriberData);
        localStorage.setItem('ktms_subscriber_data', str);
        if(snap._subscriberData.sourceFile==='업로드 Factbook' || snap._factbookUploaded){
          localStorage.setItem('ktms_factbook_uploaded', snap._factbookUploaded || str);
        }
        // in-place 뮤테이션으로 let/const 바인딩 대응
        try{
          Object.keys(subscriberData).forEach(function(k){ delete subscriberData[k]; });
          Object.assign(subscriberData, snap._subscriberData);
        }catch(ex){ subscriberData = snap._subscriberData; }
      }catch(e){}
    }
    // 헤더 배지 갱신
    const m = D.baseMonth || D.period || '2026.01';
    const badge = document.getElementById('hdrMonthBadge');
    if(badge) badge.textContent = m.replace(/[년\s]/g,'.').replace('월','').replace(/\.$/,'');
    const dm2 = document.getElementById('dashMonth2');
    if(dm2) dm2.textContent = badge ? badge.textContent : m;
  }
}
...
normalizeProfitData();
_applyInjectedSnapshot();
// 탭별 최신 저장 스냅샷 복원 (새로고침 시 각 탭 데이터 유지)
(function restoreAllTabs(){
  const active = _getTabActive();
  // profit 탭 데이터 복원 → D.profit 덮어쓰기
  const profitMonth = active['profit'];
  if(profitMonth) restoreTabSnapshot('profit', profitMonth);
  // kpi 탭 데이터 복원
  const kpiMonth = active['kpi'];
  if(kpiMonth) restoreTabSnapshot('kpi', kpiMonth);
  // tasks 탭 데이터 복원
  const tasksMonth = active['tasks'];
  if(tasksMonth) restoreTabSnapshot('tasks', tasksMonth);
  // commission 탭 데이터 복원
  const commMonth = active['commission'];
  if(commMonth) restoreTabSnapshot('commission', commMonth);
  // normalizeProfitData는 D.profit이 복원된 후 실행
  normalizeProfitData();
  // 배지: profit 기준
  updateHdrBadge(profitMonth||D.baseMonth||'2026.01');
})();
...
function renderComm1Jan(){
  ...
  if(!j||!j.mgmt_fee||!j.total_fee||!j.policy_fee||
     typeof j.total_fee.total!=='number'||typeof j.mgmt_fee.total!=='number'){
    if(el) el.innerHTML='<div style="padding:20px;color:var(--t3);font-size:12px;text-align:center">📤 BM별 이익계산서를 업로드하면 수수료 데이터가 표시됩니다</div>';
    return;
  }
  // 방어: 없는 필드 0으로 초기화
  j.total_fee.common_corp = j.total_fee.common_corp||0;
  j.total_fee.common_ch   = j.total_fee.common_ch||0;
  j.mgmt_fee.common       = j.mgmt_fee.common||0;
  j.mgmt_fee.channel_subtotal = j.mgmt_fee.channel_subtotal||j.mgmt_fee.total||0;
  j.policy_fee.rds    = j.policy_fee.rds||0;
  j.policy_fee.policy = j.policy_fee.policy||0;
  ...
  // 수수료수입 세부 항목 행 생성
  const itemRows = (Array.isArray(j.fee_items)?j.fee_items:[]).map((it,idx)=>{
  ...
  <div id="commChartSection" style="background:var(--bg2);border-radius:12px;overflow:hidden;margin-bottom:12px;border:1px solid var(--bd)">
    ...
    <div id="commChart-mgmt" style="padding:14px">
      <div style="font-size:10px;color:var(--t3);margin-bottom:12px">채널소계 ${(mgmt.channel_subtotal||mgmt.total||0).toFixed(1)}억 기준 · 공통비 ${(mgmt.common||0).toFixed(1)}억 별도</div>
      ${chans.map(ch=>{
        const v = mgmt[ch]||0;
        const pct = Math.max(2,(v/mgmtMax*100));
        const share = (mgmt.channel_subtotal||mgmt.total||0)>0?(v/(mgmt.channel_subtotal||mgmt.total)*100).toFixed(1):'0.0';
    ...
    <div id="commChart-total" style="padding:14px;display:none">
      <div style="font-size:10px;color:var(--t3);margin-bottom:12px">전사합계 ${total.total.toFixed(1)}억 · 공통비 ${((total.common_corp||0)+(total.common_ch||0)).toFixed(1)}억 포함</div>
      ${chans.map(ch=>{
        const v = total[ch]||0;
    ...
    <table style="width:100%;border-collapse:collapse;font-size:11px;min-width:560px">
      ...
      <tbody>
        ...
        <tr style="border-top:2px solid var(--bd);font-weight:700;background:var(--bg3)">
          ...
          <td style="padding:8px 8px;text-align:right;color:var(--t1)">${(total.channel_subtotal||total.total||0).toFixed(1)}</td>
        ...
        <tr style="font-weight:800;background:rgba(5,150,105,.12);border-top:1px solid #059669">
          ...
          <td colspan="7" style="padding:8px 6px;text-align:center;font-size:10px;color:var(--t3)">전사공통비(${(total.common_corp||0).toFixed(1)}) + 채널공통비(${(total.common_ch||0).toFixed(1)}) 포함</td>
...
function genCommissionReport(){
 ...
 h += `<tr style="border-top:1px dashed #cbd5e1;font-weight:600;color:#64748b"><td>채널소계</td><td style="text-align:right">${(total.channel_subtotal||total.total||0).toFixed(1)}억</td><td style="text-align:right">100%</td>

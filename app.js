...
function buildBriefingEngine(){
  ...
  // ── 가입자 데이터 통합 ──
  const SD2 = window.subscriberData || DEFAULT_SUBSCRIBER_DATA;
...
function initBriefing(){
  ...
  <div style="background:#f8fafc;border:1px solid #dbeafe;border-radius:14px;padding:16px;margin-bottom:10px">
    ...
    <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-bottom:12px" class="b-grid4">
      ...
      ${(()=>{
        const sdL=window.subscriberData||DEFAULT_SUBSCRIBER_DATA;
...
function genMonthlyBriefing(){
  ...
  // 가입자 계산
  const sdB = window.subscriberData||DEFAULT_SUBSCRIBER_DATA;
...
// ==================== (data.js에서 로드됨: DEFAULT_SUBSCRIBER_DATA, window.subscriberData) ====================
...
function saveSubscriberData(){
  try{
    const str = JSON.stringify(window.subscriberData);
    localStorage.setItem('ktms_subscriber_data', str);
    // 업로드 데이터라면 별도 키에도 저장
    if(window.subscriberData && window.subscriberData.sourceFile==='업로드 Factbook'){
...
function loadSubscriberData(){
  ...
  if(isUploaded || isVersionMatch){
    window.subscriberData = saved;
  } else {
    window.subscriberData = cloneObj(DEFAULT_SUBSCRIBER_DATA);
    delete window.subscriberData.subscriberSchema;
    saveSubscriberData();
  }
}
...
function openSubscriberReport(){
  ...
  openReport(html, `가입자 분석 보고서_${window.subscriberData.baseMonth||''}`);
}
...
function parseFactbookExcel(wb){
  try{
    ...
    window.subscriberData = extracted;
    saveSubscriberData();
    initSubscriberUI();
    initDashboard();
...
function importDataJSON(input){
  ...
  reader.onload = function(e){
    try{
      ...
      // ② window.subscriberData 복원 — 변수 할당 + localStorage 저장 동시 처리
      if(snap._subscriberData){
        window.subscriberData = snap._subscriberData;
        // sourceFile 마킹 보장
        if(!window.subscriberData.sourceFile) window.subscriberData.sourceFile = '업로드 Factbook';
        // _dataVersion 주입 (새로고침 후 loadSubscriberData가 복원하도록)
        try{ window.subscriberData._dataVersion = DEFAULT_SUBSCRIBER_DATA._dataVersion||0; }catch(ex){}
      ...
      if(statusEl) statusEl.innerHTML = '<span style="color:#86efac">✅ 불러오기 완료 · 기준월: ' + m + ' · 가입자: ' + (window.subscriberData.baseMonth||'-') + '</span>';
    }catch(err){
...
async function exportShareableHTML(){
  ...
  try{
    ...
    // ── 스냅샷 JSON 생성 (window.subscriberData + factbook 포함) ──────────
    const snap = captureSnapshot();
    try{
      const uploaded = localStorage.getItem('ktms_factbook_uploaded');
      const subRaw   = uploaded || localStorage.getItem('ktms_subscriber_data');
  ...
  if(window.__KTMS_SNAPSHOT__){
    ...
    if(snap._subscriberData){
      try{
        ...
        localStorage.setItem('ktms_subscriber_data', str);
        // 업로드 Factbook이면 전용 키에도 저장 → 새로고침 후 유지
        ...
        // window.subscriberData 변수 즉시 갱신 (initSubscriberUI 전 필수)
        window.subscriberData = snap._subscriberData;
      }catch(e){}
    }
    // 헤더 배지 갱신
...
function initDashboard(){
  try{
    ...
    const K = D.kpi || [];
    let SUB = null;
    try { if(typeof window.subscriberData!=='undefined' && window.subscriberData && window.subscriberData.wireless) SUB = window.subscriberData; } catch(e){}

  ...
  rowsR.forEach(r=>{
    ...
    if(sc>=40&&sc<=120&&rk>=1&&rk<=9&&sc>0){
      retDetail[nm]={total:sc,rank:rk,
        hubul:toNum(r[RC+3])||0,mnp:toNum(r[RC+4])||0,wired:toNum(r[RC+5])||0,mit:toNum(r[RC+6])||0,
        apru:toNum(r[RC+7])||0,strategy:toNum(r[RC+8])||0,rvoc:toNum(r[RC+9])||0,
        oneg:toNum(r[RC+10])||0,tcsi:toNum(r[RC+11])||0,
        policy_rev:toNum(r[RC+13])||0,store_profit:toNum(r[RC+14])||0};
  ...
  rowsW.forEach(r=>{
    ...
    if(sc>=40&&sc<=120&&rk>=1&&rk<=9&&sc>0){
      whlDetail[nm]={total:sc,rank:rk,
        hubul:toNum(r[WC+3])||0,mnp:toNum(r[WC+4])||0,wired:toNum(r[WC+5])||0,mit:toNum(r[WC+6])||0,
        rvoc:toNum(r[WC+7])||0,oneg:toNum(r[WC+8])||0,apru:toNum(r[WC+9])||0,
        infra:toNum(r[WC+10])||0,add_rev:toNum(r[WC+12])||0};

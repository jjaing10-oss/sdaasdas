...
function buildSubscriberAggregate(d){
  ...
  // ▼ 인덱스 기반 (month 번호가 달력 월 번호라 months.length와 불일치 방지)
  const wirelessRecs=records.filter(r=>r.channel==='무선');
  const m251=wirelessRecs[0]||{};
  const mCur=wirelessRecs[wirelessRecs.length-1]||{};
  const prev=wirelessRecs[Math.max(0,wirelessRecs.length-2)]||{};
  // YoY: 현재 월 레이블에서 전년도 같은 달 레코드 탐색 (e.g. '26.2월' → '25.2월')
  const curLabel=(months[months.length-1]||'').replace(/'/g,'').trim();

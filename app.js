...
function initSubscriberUI(){
  ...
  // 연 마감 테이블 HTML 생성
  const annualTableW_sub = renderSubscriberTable(annualWireless,[
  ...
  const annualTableW_capa = renderSubscriberTable(annualCapaW,[
    {label:'연도',align:'left',f:r=>`<b>${r.year}</b>${r._months<12?`<span style="font-size:9px;color:#94a3b8;margin-left:4px">(${r._months}개월)</span>`:''}` },
  ...
  const annualTableV_sub = renderSubscriberTable(annualWired,[
    {label:'연도',align:'left',f:r=>`<b>${r.year}</b>${r._months<12?`<span style="font-size:9px;color:#94a3b8;margin-left:4px">(${r._months}개월)</span>`:''}` },
    {label:'유지(말)',   f:r=>fmtSubNum(r.유지)},
  ...
  const annualTableV_capa = renderSubscriberTable(annualCapaV,[
    {label:'연도',align:'left',f:r=>`<b>${r.year}</b>${r._months<12?`<span style="font-size:9px;color:#94a3b8;margin-left:4px">(${r._months}개월)</span>`:''}` },
  ...
  const y24w = (d.y2024||{}).wireless||{};
  const y24v = (d.y2024||{}).wired||{};
  if(y24w.total){

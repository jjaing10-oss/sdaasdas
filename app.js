...
function parseProfitExcel(wb, filename){try{
  ...
  // 공헌이익 = 영업이익 행 기준 + 점프업 수작업 조정 (규칙①②)
  var jumpupAdj=25805899;
  var contribSrc=findInc('공헌이익')||find('영업이익');
  if(contribSrc){
    D.profit.contribution=mk(contribSrc);
  ...
  if(pfSheet){
    ...
    if(pfRevRow&&pfOpRow){
      ...
      D.profit.platform={
        ...
        total:{revenue:pfV(pfRevRow,1),contribution:pfV(pfGpRow,1),sga:pfV(pfSgaRow,1),op:pfV(pfOpRow,1)},
        teams:[
          {nm:'신사업팀',revenue:pfV(pfRevRow,2),contribution:pfV(pfGpRow,2),sga:pfV(pfSgaRow,2),op:pfV(pfOpRow,2)},
          {nm:'플랫폼사업1팀',revenue:pfV(pfRevRow,4),contribution:pfV(pfGpRow,4),sga:pfV(pfSgaRow,4),op:pfV(pfOpRow,4)},
          {nm:'플랫폼사업2팀',revenue:pfV(pfRevRow,5),contribution:pfV(pfGpRow,5),sga:pfV(pfSgaRow,5),op:pfV(pfOpRow,5)},
          {nm:'모바일파트',revenue:pfV(pfRevRow,6),contribution:pfV(pfGpRow,6),sga:pfV(pfSgaRow,6),op:pfV(pfOpRow,6)}
        ]
      };
    }
  }
...
function switchToMonth(month){
  ...
  if(!snap){ alert(month + ' 저장된 데이터 없음'); return; }
  ...
  if(dm2) dm2.textContent = month;
  // 전체 UI 재초기화
  initTaskUI(); initProfitUI(); initKpiUI(); initDashboard();
  try{ initCommission(); }catch(e){}
  // subscriber는 별도 localStorage 키 사용 중 — 그대로 유지
  closeMonthPicker();
}
...
function renderMpMonthGrid(){
  ...
  el.innerHTML = months.map(m=>{
    ...
    return `<div class="mp-m${isActive?' active':''}${hasData&&!isActive?' has-data':''}" onclick="switchToMonth('${m}')">${label}</div>`;
  }).join('');
}
...
function renderMpSavedList(){
  ...
  el.innerHTML = stored.reverse().map(m=>{
    ...
    return `<div class="mp-saved-item">
      <div>
        <div class="mp-saved-month">${m}</div>

...
function parseKpiExcel(wb, filename){try{
  ...
  rowsG.forEach(r=>{
    if(!isHqRow(r)) return;
    kpi.push({hq:hqName(r[1]),ts:toNum(r[2])||0,rk:toNum(r[3])||0,
      rt:{t:+r[4]||0,s:+r[5]||0,p:+r[6]||0},
      wh:{t:+r[7]||0,s:+r[8]||0,p:+r[9]||0},
      sm:{t:+r[10]||0,s:+r[11]||0,p:+r[12]||0}});
  });
  ...
  rowsR.forEach(r=>{
    ...
    const sc=toNum(r[2])||0; const rk=toNum(r[3])||0;
    if(sc>=40&&sc<=120&&rk>=1&&rk<=9&&nm.indexOf(' ')<0&&sc>0){
      retDetail[nm]={total:sc,rank:rk,
        hubul:+r[4]||0,mnp:+r[5]||0,wired:+r[6]||0,mit:+r[7]||0,
        apru:+r[8]||0,strategy:+r[9]||0,rvoc:+r[10]||0,
        oneg:+r[11]||0,tcsi:+r[12]||0,
        policy_rev:+r[14]||0,store_profit:+r[15]||0};
    } else if(sc>=40&&sc<=110&&rk>=1&&rk<=20&&nm.indexOf(' ')<0&&sc>0){
  ...
  rowsW.forEach(r=>{
    ...
    if(sc>=40&&sc<=120&&rk>=1&&rk<=9&&nm.indexOf(' ')<0&&sc>0){
      whlDetail[nm]={total:sc,rank:rk,
        hubul:+r[4]||0,mnp:+r[5]||0,wired:+r[6]||0,mit:+r[7]||0,
        rvoc:+r[8]||0,oneg:+r[9]||0,apru:+r[10]||0,
        infra:+r[11]||0,add_rev:+r[13]||0};
    } else if(sc>=40&&sc<=120&&rk>=1&&rk<=20&&nm.indexOf(' ')<0&&sc>0){
  ...
  rowsS.forEach(r=>{
    ...
    if(!nm||nm.length<2) return;
    const sc=toNum(r[2])||0; const rk=toNum(r[3])||0;
    if(!(sc>=40&&sc<=120&&rk>=1&&rk<=9&&nm.indexOf(' ')<0)) return;
    smbDetail[nm]={total:sc,rank:rk,
      hubul:+r[5]||0,wired:+r[6]||0,mit:+r[7]||0,
      strategy:+r[8]||0,rvoc:+r[9]||0,op_profit:+r[10]||0};
  });

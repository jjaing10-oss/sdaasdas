...
function parseKpiExcel(wb, filename){try{
  ...
  // ── 헬퍼: 본부행 판별 - 문자열 비교 대신 숫자 패턴 사용 ──
  // 본부행: r[1]=문자열, r[2]=KPI점수(50~110), r[3]=순위(1~9)
  const isHqRow = function(r){
    var nm = hqName(r[1]);
    if(!nm || typeof r[2]!=='number' || typeof r[3]!=='number') return false;
    return r[2]>=40 && r[2]<=120 && r[3]>=1 && r[3]<=9 && nm.length>=2;
  };
  // ── 헬퍼: 지사행 판별 - 지사는 r[3]이 1~15 범위 순위, r[2]가 점수 ──
  const isJisaRow = function(r,suffix){
    var nm = hqName(r[1]);
    if(!nm || typeof r[2]!=='number') return false;
    if(suffix) return nm.slice(-suffix.length)===suffix;
    return r[2]>=40 && r[2]<=120 && (r[3]>=1||r[3]==='-');
  };
  ...
  rowsG.forEach(r=>{
    if(!isHqRow(r)) return;
    kpi.push({hq:hqName(r[1]),ts:+r[2]||0,rk:+r[3]||0,
      rt:{t:+r[4]||0,s:+r[5]||0,p:+r[6]||0},
      wh:{t:+r[7]||0,s:+r[8]||0,p:+r[9]||0},
      sm:{t:+r[10]||0,s:+r[11]||0,p:+r[12]||0}});
  });
  if(kpi.length<3){showUpStatus('kpi','err','❌ KPI 데이터 부족 (종합 '+kpi.length+'개 / 행'+rowsG.length+') 시트목록: '+wb.SheetNames.join(', '));return;}
  ...
  rowsR.forEach(r=>{
    const nm=hqName(r[1]);
    if(!nm||typeof r[2]!=='number') return;
    const sc=+r[2]||0; const rk=+r[3]||0;
    if(sc>=40&&sc<=120&&rk>=1&&rk<=9&&nm.length>=2&&!nm.includes(' ')&&sc>0){
    ...
    } else if(sc>=40&&sc<=110&&rk>=1&&rk<=20&&nm.length>=2&&!nm.includes(' ')&&sc>0){
  ...
  rowsW.forEach(r=>{
    ...
    } else if(sc>=40&&sc<=120&&rk>=1&&rk<=20&&nm.length>=2&&!nm.includes(' ')&&sc>0){
  ...
  rowsS.forEach(r=>{
    ...
    if(!nm||typeof r[2]!=='number') return;
    const sc=+r[2]||0; const rk=+r[3]||0;
    if(!(sc>=40&&sc<=120&&rk>=1&&rk<=9&&nm.length>=2&&!nm.includes(' '))) return;

...
function buildSubscriberAggregate(d){
  ...
  // ── 헤더 배지(D.baseMonth) 또는 직접 선택(__subMonthIdx) 기준으로 표시 월 인덱스 결정 ──
  let idx = months.length - 1; // default: 마지막 월
  try{
    // ① 드롭다운에서 직접 선택한 경우 우선
    if(typeof window.__subMonthIdx==='number' && window.__subMonthIdx>=0 && window.__subMonthIdx<months.length){
      idx = window.__subMonthIdx;
    } else {
      // ② 헤더 배지(D.baseMonth) 기준
      const badge = (typeof D!=='undefined' && D.baseMonth) ? String(D.baseMonth) : null;
      if(badge){
        const bm = badge.match(/^20(\d{2})\.0?(\d+)/);
        if(bm){
          const norm = `${bm[1]}.${parseInt(bm[2])}월`; // '26.1월'
          const fi = months.findIndex(m=>m.replace(/'/g,'').trim()===norm);
          if(fi >= 0) idx = fi;
        }
      }
    }

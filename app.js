...
function parseKpiExcel(wb, filename){try{
  ...
  const detectC = function(rows){
    for(var i=0;i<rows.length;i++){
      var r=rows[i];
      // r[0]이 본부/지사명 패턴이고 r[1]이 KPI점수면 C=0
      if(typeof r[0]==='string' && r[0].length>=2 && r[0].indexOf(' ')<0){
        var s=toNum(r[1]); if(s!==null&&s>=40&&s<=120) return 0;
      }
      // r[1]이 본부/지사명 패턴이고 r[2]가 KPI점수면 C=1
      if(typeof r[1]==='string' && r[1].length>=2 && r[1].indexOf(' ')<0){
        var s2=toNum(r[2]); if(s2!==null&&s2>=40&&s2<=120) return 1;
      }
    }
    return 1; // 기본값
  };

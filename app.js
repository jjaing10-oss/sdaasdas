...
async function exportShareableHTML(){
  ...
  try{
    ...
    // ── file:// 모드: DOM outerHTML 기반 내보내기 ─────────────────
    if(location.protocol === 'file:'){
      const snapJson = JSON.stringify(snap);
      const lsMeta = JSON.stringify(_getMonthMeta());
      const lsTab  = JSON.stringify(_getTabActive());
      const fbUp   = JSON.stringify(localStorage.getItem('ktms_factbook_uploaded')||'');
      const injectScript = '<scr'+'ipt>window.__KTMS_SNAPSHOT__='+snapJson+';'
        +'try{localStorage.setItem("'+MONTH_META_KEY+'",'+JSON.stringify(lsMeta)+');}catch(e){}'
        +'try{localStorage.setItem("'+TAB_ACTIVE_KEY+'",'+JSON.stringify(lsTab)+');}catch(e){}'
        +'try{var _fu='+fbUp+';if(_fu)localStorage.setItem("ktms_factbook_uploaded",_fu);}catch(e){}'
        +'<\/scr'+'ipt>';
    ...
    // ── 서버 모드: 소스 파일 fetch 후 완전 인라인 ─────────────────
    const [appJs, styleCss, dataJs, indexHtml] = await Promise.all([
      fetch('app.js').then(r=>r.ok?r.text():null).catch(()=>null),
      fetch('style.css').then(r=>r.ok?r.text():null).catch(()=>null),
      fetch('data.js').then(r=>r.ok?r.text():null).catch(()=>null),
      fetch('index.html').then(r=>r.ok?r.text():null).catch(()=>null),
    ]);
    ...
    const lsRestore = '<scr'+'ipt>try{'
      +'localStorage.setItem("'+MONTH_META_KEY+'",'+JSON.stringify(JSON.stringify(_getMonthMeta()))+');'
      +'localStorage.setItem("'+TAB_ACTIVE_KEY+'",'+JSON.stringify(JSON.stringify(snap._tabActive||{}))+');'
      +'var _fu='+fbUp+';if(_fu)localStorage.setItem("ktms_factbook_uploaded",_fu);'
      +'}catch(e){}<\/scr'+'ipt>';
...
function _applyInjectedSnapshot(){
  if(window.__KTMS_SNAPSHOT__){
    ...
    // subscriber 데이터 복원
    if(window.__KTMS_SNAPSHOT__._subscriberData){
      try{ localStorage.setItem('ktms_subscriber_data', JSON.stringify(window.__KTMS_SNAPSHOT__._subscriberData)); }catch(e){}
    }
    // 헤더 배지 갱신

...
function parseProfitExcel(wb, filename){try{
  ...
  try{
    ...
    if(feeData.total_fee){
      ...
      // 관리수수료
      if(feeData.mgmt) j26.mgmt_fee={
        total:+feeData.mgmt.total.toFixed(2),channel_subtotal:+feeData.mgmt.total.toFixed(2),
...
function parseKpiExcel(wb, filename){try{
  ...
  // ── 헬퍼: 시트명 정규화 비교 ──
  const normSheet = s => String(s||'').replace(/\s/g,'').toLowerCase();

  // ── 헬퍼: 시트→rows (이름 직접 + 키워드 fallback + 인덱스 fallback) ──
  const sheetRows = function(name){
    // 1) 직접 이름 매칭
    let ws = wb.Sheets[name];
    // 2) 공백 제거 후 includes 매칭
    if(!ws){
      const kw = normSheet(name.replace(/^1Q/,''));
      const found = wb.SheetNames.find(s=>normSheet(s).includes(kw));
      if(found) ws = wb.Sheets[found];
    }
    // 3) 순서 기반 fallback: '1Q 종합'은 첫번째, '1Q 소매'는 두번째...
    if(!ws){
      const orderMap = {'1Q 종합':0,'1Q 소매':1,'1Q 도매':2,'1Q 소상공인':3};
      const idx = orderMap[name];
      if(idx!==undefined && wb.SheetNames[idx]) ws = wb.Sheets[wb.SheetNames[idx]];
    }
    return ws ? XLSX.utils.sheet_to_json(ws,{header:1,defval:0}) : [];
  };

  // ── 헬퍼: 본부명 정규화 ──
  const hqName = v => String(v||'').trim();
  // ── 헬퍼: 본부행/지사행 판별 ──
  const isHqRow = r => hqName(r[1]).endsWith('\uBCF8\uBD80');  // '본부'
  const isJisaRow = (r,suffix) => { const n=hqName(r[1]); return suffix?n.endsWith(suffix):(n.includes('\uC18C\uB9E4')||n.includes('\uB3C4\uB9E4')); };
  ...
  if(kpi.length<3){showUpStatus('kpi','err','\u274C KPI \uB370\uC774\uD130 \uBD80\uC871 (\uC885\uD569 '+kpi.length+'\uAC1C / \uD589'+rowsG.length+' / \uC2DC\uD2B8:'+sheetDbg+')');return;}

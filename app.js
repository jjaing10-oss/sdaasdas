  ...
  reader.onload = function(e){
    try{
      ...
      if(snap._subscriberData){
        ...
        try{ localStorage.setItem('ktms_subscriber_data', str); }catch(ex){}
        if(snap._factbookUploaded){
          try{ localStorage.setItem('ktms_factbook_uploaded', snap._factbookUploaded); }catch(ex){}
        } else if(snap._subscriberData.sourceFile==='업로드 Factbook'){
          try{ localStorage.setItem('ktms_factbook_uploaded', str); }catch(ex){}
        }
      }
      // 헤더 배지
      const m = snap._exportMonth || D.baseMonth || '2026.01';
      D.baseMonth = m;
      updateHdrBadge(m);
      // 전체 UI 갱신
      ...
      if(statusEl) statusEl.innerHTML = '<span style="color:#86efac">✅ 데이터 불러오기 완료 · 기준월: ' + m + '</span>';
    }catch(err){

...
function parseKpiExcel(wb, filename){try{
  ...
  // ── 컬럼 오프셋 자동 감지: 본부명이 r[0]인지 r[1]인지 ──
  var _colOff = 1; // 기본: r[1]=본부명
  for(var _ri=0;_ri<rowsG.length;_ri++){
    var _r=rowsG[_ri];
    var _sc0=toNum(_r[1]); var _sc1=toNum(_r[2]);
    if(typeof _r[0]==='string' && _r[0].length>=2 && _sc0!==null && _sc0>=40 && _sc0<=120){ _colOff=0; break; }
    if(typeof _r[1]==='string' && _r[1].length>=2 && _sc1!==null && _sc1>=40 && _sc1<=120){ _colOff=1; break; }
  }
  const C = _colOff; // C=0: r[0]=이름, C=1: r[1]=이름
  // ── 헬퍼: 본부행 판별 ──

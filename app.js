...
async function exportShareableHTML(){
  ...
  try{
    await _fetchSources();
    if(!_cachedAppJs){ alert('소스 파일 로드 실패. 파일로 직접 열기 환경에서는 공유 내보내기가 지원되지 않습니다.'); return; }
    ...
    // 현재 index.html fetch
    let indexHtml = await fetch('index.html').then(r=>r.text()).catch(()=>null);
    if(!indexHtml){ alert('index.html 로드 실패'); return; }

    // 외부 참조 → 인라인으로 교체
    const inlineStyle = `<style>${_cachedStyleCss}</style>`;
    const inlineApp = `<script>${_cachedAppJs}<\/script>`;

    // 스냅샷 주입 스크립트 (data.js 로드 후, app.js 로드 전 실행)
    const snapScript = `<script>
window.__KTMS_SNAPSHOT__ = ${snapJson};
<\/script>`;

    // link/script 교체
    indexHtml = indexHtml
      .replace(/<link[^>]*href="style\.css"[^>]*>/i, inlineStyle)
      .replace(/<script[^>]*src="data\.js"[^>]*><\/script>/i, `<script src="data.js"><\/script>${snapScript}`)
      .replace(/<script[^>]*src="app\.js"[^>]*><\/script>/i, inlineApp);

    // 저장된 월 메타도 embed
    const metaScript = `<script>try{localStorage.setItem('${MONTH_META_KEY}',${JSON.stringify(JSON.stringify(_getMonthMeta()))});localStorage.setItem('${MONTH_STORE_PREFIX}${D.baseMonth||'2026.01'}',${JSON.stringify(snapJson)});}catch(e){}<\/script>`;
    indexHtml = indexHtml.replace('</body>', metaScript + '</body>');

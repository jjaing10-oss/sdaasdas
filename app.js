...
function parseTaskExcel(wb, filename){
  ...
  if(tasks.length>0){D.tasks=tasks;initTaskUI();initDashboard();showUpStatus('task','ok','✅ '+tasks.length+'개 과제 로드!'+(month?` · 기준월 ${month}`:''));autoSaveCurrent();}
...
function autoSaveCurrent(){
  // 업로드 성공 후 자동 저장
  setTimeout(()=>saveMonthToStorage(D.baseMonth||'2026.01'), 300);
}

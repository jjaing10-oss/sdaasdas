...
function saveTabSnapshot(tab, month){
  ...
  keys.forEach(k=>{ if(D[k]!==undefined) snap[k]=D[k]; });
  try{ localStorage.setItem(TAB_SNAP_PREFIX+tab+'_'+month, JSON.stringify(snap)); }catch(e){}
...
function autoSaveCurrent(tab){
  // tab: 'profit'|'kpi'|'tasks'|'commission' (없으면 전역 저장만)
  const month = D.baseMonth||'2026.01';
  setTimeout(()=>{
    saveMonthToStorage(month); // 기존 전역 저장도 유지
...
(function restoreAllTabs(){
  ...
  ['profit','kpi','tasks','commission'].forEach(tab=>{
    const month = active[tab];
    if(month) restoreTabSnapshot(tab, month);
  });
  normalizeProfitData();
  // 현재 탭(dashboard 기본)의 배지는 profit 월 우선
  const profitMonth = active['profit']||D.baseMonth||'2026.01';
  updateHdrBadge(profitMonth);
})();

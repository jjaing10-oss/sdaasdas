...
function switchTab(btn){
  ...
  if(tabMonth && TAB_DATA_KEYS[t]){
    ...
    // 대시보드·브리핑 등 자체 데이터 없는 탭 → 저장된 최신 월 표시
    const storedMonths = getStoredMonths();
    const latestM = storedMonths.length ? storedMonths[storedMonths.length-1] : null;
    if(latestM && latestM !== D.baseMonth){
      const latestSnap = loadMonthFromStorage(latestM);
      if(latestSnap){ restoreSnapshot(latestSnap); normalizeProfitData(); }
    }
